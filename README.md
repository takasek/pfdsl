...
The GitHub-Issues backend's audit scripts require the `gh` CLI, authenticated to your repo's GitHub host. Earlier versions documented a token-only setup (`GH_TOKEN` / `GITHUB_TOKEN` without `gh`); that setup is no longer supported, so install `gh` and authenticate it when you update. The runtime requirements are listed in the `pfd-ops` skill's `references/github-issues-backend.md`.
