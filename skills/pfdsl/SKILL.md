...
Full flag reference: `npx @pfdsl/cli help`. This section documents CLI v0.0.16. If a command above is reported as `unknown command`, the installed/published CLI is older than this skill — check `npx @pfdsl/cli@latest help`.
...
`references/spec.md` は full spec v0.0.14（20節・大型）— 全読せず、節見出し（`## N.`）とエラーコードで該当箇所だけ読む。
