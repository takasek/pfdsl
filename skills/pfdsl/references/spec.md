...
`tags` は Artifact / Process の両方に指定してよい（`group` §2.8 と対称）。似た構造・振る舞いを持つ Process 群を共通タグで束ね、その族に関する共有知識を companion `.md` に外化する用途を想定する（グラフは各 Process を独立ノードとして保ったまま、共有知識は散文へ逃がす）。
...
* 可視化バックエンドは `tagStyles` を Artifact / Process いずれの node にも適用してよい（status は Artifact 専用）
...
1. tags 配列を逆順走査し、tagStyles[tag] を順次マージ（後マージ勝ち = 先頭タグ最終勝者）。Artifact / Process の両方に適用する
...
* §2.7.2 `tags` を Artifact / Process の両方に許可（`group` §2.8 と対称）。可視化は `tagStyles` を Process node にも適用する（status は Artifact 専用のまま）
