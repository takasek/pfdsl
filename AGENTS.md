...
変更束はブランチで作業し PR で main に統合する（main 直コミットしない。生態系図の develop→PR→merge_pr が正規経路）。`scripts/main-commit-guard.mjs`（PreToolUse(Bash) hook、`.codex/hooks.json` で配線）は、直接記述された既知のshell形についてmainブランチまたはsibling worktreeのツリー・インデックスを変えるgitコマンドを止める事故防止guardである（#650・#777・#784）。任意のBash source・展開・関数・別scriptを完全に解釈するsecurity boundaryではない。
