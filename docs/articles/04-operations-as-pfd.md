...
**選択**では、着手できるプロセスの列挙と推薦を、第1部で見た`ready`コマンドが返す。
どの候補を選ぶかの最終判断と、選んだプロセスの一次情報（issue本文など）を読む作業はAIが担う。
...
進捗の更新は`status-set`コマンドが担い、statusを書き換えると同時に、その変更で新たに解放された着手可能プロセスを列挙して返す。
更新の副作用（何が着手できるようになったか）を、更新した本人が頭の中で追い直す必要がない。
図の構造は`check`が検査する。
`check --audit`は終端の成果物を列挙し、仕様や計画のような手段の成果物が、それを消費する後続を持たないまま終端に残っていないかを監査する。
roadmapと他の図とのずれは`audit-sync`が突合し、進行管理に載っていない作業項目を検出する。
...
この分業は、第2部で見た二層分業（設計判断は人が下し、規則への適合確認は機械に任せる）の運用版である。
