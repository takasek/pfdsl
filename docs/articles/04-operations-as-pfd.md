...
| 実行 | 構造の検査（`check`） | 作業そのもの（コード・文書の作成） |
...
要件から仕様書を作ったところで止まっている図にかけると、こう返る。

```
$ pfdsl check roadmap.pfdsl --audit
...
さらに、roadmapと他の図とのずれは`audit-sync`が突合し、進行管理に載っていない作業項目を検出する。
