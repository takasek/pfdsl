...
固有層のうち、issueを切るかどうか、いつ閉じるかといった管理の流儀は、リポジトリごとに書き起こすのではなく選んで載せるプリセットとして扱い、差し替えれば同じ運用ハーネスが別の流儀のリポジトリでも動く。
...
**選択**では、機械が返した列挙と推薦（第1部で見た`ready`）を受けて、どの候補を選ぶかの最終判断と、選んだプロセスの一次情報（issue本文など）を読む作業をAIが担う。
複数の実装方針が列挙されたまま選択が明記されていない作業は、設計が未確定である。
その場合は実装に入らず、先に方針を確定させる。
この見極めは文章の解釈であり、機械には任せられない。

**実行**は、作業そのもの、つまりコードを書き文書を起草する段階であり、AIの文脈と判断力はここに使う。
...
specの完成でimplementに着手できるようになった、という更新の副作用を、更新した本人が頭の中で追い直す必要がない。
...
たとえばworkflowに新しい成果物を追加したのにroadmapへの登録を忘れている、という食い違いが検出される。
一方、作業で得た知見をどの文書に振り分けるか、作業中に見つかった新しい成果物をどう図に足すかは、AIが判断する。
ビルドに新しい前提条件が見つかったなら図の隣の説明ファイルへ、レビューの過程で新しい文書が必要だと分かったなら図そのものへ、という振り分けである。
