...
コーディングエージェントの使い方に関する議論の重心は、1回ずつ指示を出してやり取りすることから、エージェントが停止条件を満たすまで作業サイクルを繰り返す仕組み、つまりループをどう設計するかへ移りつつある。
Anthropicはループの始め方を公式ブログで解説しており、そこではループが起動の契機と停止条件で4種類に分類される[^loops]。
人がプロンプトを出すたびに1周が回るターンベース、あらかじめ与えた完了条件を満たすまで回り続けるゴールベース、一定間隔で起動する時間ベース、イベントを契機に人の介在なしで動くプロアクティブである。
...
- **状態の置き場**：図とstatusはgitに載ったテキストであり、セッションの外にある。だから各周回は文脈ゼロのセッションでよく、前の周回の記憶を引き継ぐ必要がない。
...
実行者の自己申告に頼らず、振り返りは監査の観点表で図とセッションを点検する。
