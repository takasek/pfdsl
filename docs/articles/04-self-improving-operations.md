...
しかも人間の新規参加者なら一度覚えた慣習は次回に持ち越されるが、AIエージェントはセッションが終わると説明の内容を保持しない。
...
変換の境界がどこにあるかを確認しながら実装するときに効く。

このうち、プロジェクトの進行管理を担うのはroadmapとworkflowである。
...
roadmapとworkflowは、スキル（AIエージェント向けの再利用可能な手順書）が読んで回す。
...
前の周回の記憶を引き継ぐ必要がないことは、先ほどの報告の段階で見たとおりである。
...
ループを回す機構と、ループが読む状態の記述は、別々に選べる部品なのだ。
...
## リポジトリに依存しない層と、リポジトリに蓄積する層

このループは、それを育てたリポジトリの専用品ではない。
同じ運用ハーネスを別のリポジトリに持ち込むと、そこでもループが回る。

持ち込んだ先では、AIが進行管理の図を新規に構築する。
...
このあいだ、手順書そのものには手を入れていない。

手を入れずに済むのは、運用ハーネスが二つの層に分かれているからだ。
汎用の層は、PFDの概念だけから導ける手順でできている。
着手の判断は「入力が全てdone」で下す。
進捗はstatusの更新で報告する。
1サイクルでは1プロセスだけを進める。
これらはグラフの意味論だけで決まるため、どのリポジトリでも同じ手順書がそのまま使える。
リポジトリ固有の中身は、手順書ではなく別の場所に置く。
テストの回し方やビルドのコマンドは図の隣の説明ファイルへ。
issueを切るかどうか、いつ閉じるかといった管理の流儀は、選んで載せるプリセットへ。
手順書は「隣の説明ファイルに従え」とだけ書いておく。

層が分かれているから、配布の形も決まる。
汎用の層は、Claude Codeのpluginとして配布されている。
...
導入した先では、初期化のコマンドが対象プロジェクトに図の雛形を配り、以後の運用はこの部で見たサイクルに乗る。
一方、リポジトリ固有の層は配布されない。
進行の状態はroadmapに、繰り返しの手順はworkflowに、規約や知見は図の隣の説明ファイルに、使いながら書き足されていく。
運用ハーネスを使い込むほど、配布された手順書ではなく、対象プロジェクトの側に知見が積み上がる。
...
ただし、外化されたのは判断の材料までである。
完了の判定は宣言済みの基準に照らして読み手が下し、成果物の粒度は委譲の失敗が更新していく。
