...
既存の依存グラフに一続きの依存を1本追加するだけで、その提案が仕様の他の起草作業と並列に進められること、既存方針との接点が1箇所だけであること、最後は仕様を1つにまとめる統合プロセスに合流することが確定する。
...
担当者は着手前にこの図を読むだけで、二つの提案が独立ではなく決定が行き来しながら形成される関係にあるという作図者の判断を受け取り、二つを別々に進める誤りを避けられる。
