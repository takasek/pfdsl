...
8件のissueが、互いを待たずに進められる9本の作業に分解される、といった答えが即座に出る（1件のissueが複数の作業に割れることがあるため、数は一致しない）。
...
この種の境界ケースを洗い出す作業を8通りの組み合わせについて行うと（1通りから複数のケースが出るものもある）、そのまま11件のテストケースとして使える具体例の集合になった。
