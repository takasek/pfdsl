...
- `[a, b] >> P`：集合記法。
- frontmatterで成果物にstatus（done / wip / todoなど）を付けられる。
...
図の更新がコードの変更と同じ営みに乗り、CIの検査対象にもなる。
...
```
$ node packages/cli/dist/cli.js check dup.pfdsl
...
この検査をコミット時やCIの確認事項に組み込めば、図の腐敗、つまり実体との乖離や構造の壊れをコミットのタイミングで検出できるようになる。
図を後から見直す作業ではなく、変更が入るたびに通る関門になる。
同じ方向を先に歩いていた例として、DeNAのSWETが公開しているPFD作図ツールがある[^swet]。
図をテキストで管理して差分やレビューに乗せるdiagram-as-codeという潮流とも軌を一にする[^diagram-as-code]。

## 書くこと自体が構造を暴く

検査にかける前の段階にも効用がある。
構造化された言語で書くこと自体が、思考のフレームワークとして働く。
書いた直後に検査や描画のフィードバックが返ってくるので、PFDは結果を記録する道具である前に、考えを整理する道具になる。
これはDSLの本質的な性質であり、AIと一緒に考えるときはこの強みがいっそう効いてくる。
