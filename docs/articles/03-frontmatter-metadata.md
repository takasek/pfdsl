...
メタデータが効力を発揮するのは、図を読むだけの場面ではなく、運用に必要な情報へアクセスし更新する場面である。
...
広げた直後に自分が管理している進行管理の図へ検査をかけると、作業中の項目のうち完了条件を書き忘れているものが何件も引っかかる。
ルールを作った本人の図が、ルールを適用した瞬間に真っ先に指摘される側に回る。
...
どちらもグラフの意味論には影響しない。
着手可能集合の計算やstatusの伝播はこれらのフィールドの有無に左右されない。
...
形式化されたfrontmatterは、道具だけでなく進行を回す手順書からも読める。
その仕組みは第4部で扱う。
...
図の編集自体に関する規則も同様である。
「あるプロセスが公開物を作る場合、そのプロセスの入力の列挙は、公開される実装成果物が追加されたのと同じサイクルで更新する」という規則を考える。
これを怠ると、追加した実装成果物が公開のチェーンから切れたまま残り、気づかれずに放置される。
...
この3つは種類が異なる。
1つ目は全ノードに横断する規約、2つ目は図の編集手順そのものの規則、3つ目は外部システムとの接続の宣言である。
共通するのは、いずれも1ノードの属性として書けない、という一点である。
frontmatterをノード単位の属性に限定したことの裏返しとして、こうした情報の置き場所が別に要る。

## 払うコスト

`criteria`の検査が保証するのは、フィールドが存在することだけである。
書かれた文言の質までは検査できない。
