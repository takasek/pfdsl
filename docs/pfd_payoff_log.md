...
- 効果: 着手時点で graph から「#5/#6 は決定が往復で形成される相互依存 → 分割せず共同設計、共有決定を multifile_policy として外化」が直読でき、review-prompts B「並列主張への挑戦」の罠（共有決定を持つ作業群を並列と誤認）を自力検出せずに済んだ。設計対話も「2 issue を別々に起草」でなく「共有決定（ID スコープ・継承解決順・ファイル間参照）を先に確定 → 各提案はそれを参照」と最初から正しい構造で進められた。反実仮想: roadmap が #5/#6 を独立 todo として並べていたら、相互依存に気付くのは起草が衝突してからだった
- 学習: B「並列主張」は本来レビュー時の問いだが、roadmap 設計時に相互依存を1プロセス複数出力へ畳んでおくと、後続サイクルの着手者がその問いを踏む前に正解構造を受け取れる。図が「書くときのルール」を内包して「使うときの罠」を消した実例
- 参照: PR #126、roadmap.pfdsl `draft_multifile_specs`、docs/spec/proposals/multifile-policy.md、review-prompts B「並列主張への挑戦」、ADR-0004 基準3
