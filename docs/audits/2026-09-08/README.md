...
この資料は、次回同種の監査を行う際に、対象コミット・反例・観測結果をまとめて参照し、再現・比較するために保存する。
複数領域の報告、再現スクリプト、結果、根拠となるログを相互に参照できるよう、Git で一体として管理する。
[削減方針](reduction-plan.md)も、当時の判断の経緯を示す記録として保持する。

現在の残課題・優先順位・対応状況は [#1055](https://github.com/takasek/pfdsl/issues/1055)、具体的な実装範囲と受入条件は着手する個別 issue で管理する。
再監査では [再現手順と証拠](evidence/README.md)および所見 ID を手がかりに、新しい対象コミットでの結果を比較する。
ここにある観測値と判断は監査時点の記録として保持し、新しい結果は対象コミットとともに別途記録する。
