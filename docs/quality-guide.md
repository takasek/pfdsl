...
- **進捗 status は roadmap にだけ書く**: `status:` は計画の状態を答えるフィールドであり、その問いを持つ図は roadmap である。workflow / runtime-pipeline は手続きを描く図なので artifact に `status:` を書かない。同一 id が複数の図に現れる場合も一次情報は roadmap 側に一元化する — 両方に書くと2つの図が同じ現実について別の状態を主張でき、どちらが正かを図からは決められない
...
- **criteria は判定できる形で書く**: 満たされているかを実行または観察で判定できる形にする。判定手段を名指す場合、その手段が実際に返す値を確かめてから書く — 名前が示唆する戻り値と実際の戻り値は別物で、確かめずに書いた criteria は書いた時点では真偽が決まらない。最新の1件だけを返す手段で特定の値を主張すると、最新を指す1件を除いて文字どおり偽になる
