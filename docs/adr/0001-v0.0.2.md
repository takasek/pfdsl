...
title: 開発フロー version: 1.2 dsl_version: 0.0.2 tags: [web, review]
...
process:
impl:
title: 実装 owner: dev-team estimate: 5d
...
req >> design -> spec spec >> impl -> code code >> test -> release

17.2 フィードバック

code >> review -> issues issues >>? impl -> code
...
customer_bug_report >>? impl market_feedback >>? design audit_result >>? operation
...
raw_data >> build -> report template >> build -> report
...
test >> judge -> success test >> judge -> failure
...
[a,b] >> merge -> c [a,b] >> compile -> c [a,b] >> package -> c
