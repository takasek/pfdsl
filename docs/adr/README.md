...
- **ADR-0036** [配布層 probe の起動条件を公開境界に置く](0036-probe-trigger-at-publish-boundary.md) — ADR-0029 は手法を決めたが「いつ回すか」を書かず、1回実施されたきり休眠した。公開版と probe 記録の版を比較し、記録が古ければ cli リリースの pre-tag checks を落とす。サイクル境界に置かないのは、配布層に触る頻度では記録が形式的充足へ劣化するため。実績があるのは観点1（grep 監査、#417 を検出）であり fresh agent 観点は同じ回に見逃しているため、観点1 の主要規則は静的検査へ移し probe には観点2・4 を残す
