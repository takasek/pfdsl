...
- **ADR-0032** [pfd-ops 配布物を専用ディレクトリ `scripts/pfdsl/` に隔離する](0032-pfd-ops-install-dedicated-dir.md) — 採用リポの `scripts/lib/gh-exec.mjs` 等は pfdsl 由来か自リポ独自か区別がつかなかった。ワークフロー本体を薄い呼び出しに縮小し composite action化することで、実体スクリプトを `scripts/pfdsl/` へ集約する。ADR-0016 が過去に却下した「composite action 経由」は canonical-in-repo 前提のもとでの判断であり、ADR-0028 がその前提を崩したことで再評価が可能になった
