...
4. **唯一の明示的例外**: `meta set` は roadmap ファイルに対して newly-ready になったプロセスの報告を出力する。
   ...
   ただし `meta set` 自体は ADR-0022 の gofmt 出力モデル（既定 stdout プレビュー／`--write`／`--check` の三点セット）が対象とする「ファイル全体の変換」の対象外に意図して置かれている。
   指定した id・フィールドだけを狙い撃ちする命令的な変更操作であり、ファイル全体を変換するコマンドではないため、既定 stdout プレビューモードを持たない。
