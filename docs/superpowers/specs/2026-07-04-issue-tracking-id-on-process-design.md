...
- 1 processが複数出力artifactを持つ場合の「対象artifact」特定方法（現行roadmapでの実例有無を確認する）
- `flow-on-issue-close.yml` の自動PRが今回の移行作業中に発火した場合の扱い（旧ロジックのままCIが動く可能性があるため、audit script更新とroadmap移行を同一PRでまとめて扱うか順序を決める）
