...
- [x] Track protected Git and CDPATH state through the supported assignment, export, unset, read, and printf forms, including escaped argv, option termination, conflicting options, dynamic output names, and array output variables; restore known-safe state only after a literal operation proves it.
- [x] Fail closed for target-affecting OR-lists, pipelines, background commands, subshells, conditional lists, and unsupported compounds while preserving direct success-path AND-lists, target-neutral pipelines and AND-lists, and later unrelated control flow.
- [x] Reproduce the bounded adversarial matrix against the real hook, close its concrete Important findings, and stop treating incremental shell parsing as an arbitrary-Bash security boundary.
- [x] Document the shared repository hook as a direct-form accident guard, keep the Codex mechanical boundary in the trusted wrapper plus sandbox, and avoid imposing the Codex-only user wrapper on Claude Code.
- [x] Run the focused guard suites and synchronize the design, workflow PFD, and generated harness guidance.
