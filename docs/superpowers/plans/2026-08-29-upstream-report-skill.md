...
- **一時ファイル**: 照合の成功後に削除し、失敗して停止した場合は body を残す（手動投稿の入力になるため）
- **重複確認の失敗**: 検索そのものが失敗した場合は確認を省いて起票へ進まず停止する
- **pfd-retro の既定**: 自リポが上流の場合も、既定はその場修正ではなく pfd-upstream-report での起票とした
- **ヘルパーの再利用**: `readJsonOrNull` は `plugin-version-check.mjs` の同一実装を export して共有する。リポルートの遡上は `check-install-sync.mjs` にもあるが、あちらは見つからないと起点を返すため `unknown` 形態を表現できない。共有せず `findRepoRootOrNull` と名付けて違いを名前に出した（注入オプション名も同じ）
