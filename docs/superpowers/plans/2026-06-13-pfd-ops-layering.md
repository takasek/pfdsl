...
- Modify: `.pfdsl/ecosystem.pfdsl` — `ecosystem.md` / `roadmap.md` の artifact 登録、`ops_skill` description 更新
...
設計 §3 手順5。再利用可能な GitHub Issues バックエンド規約と採用手順。リポ固有のインスタンス値（実パス・URL）は含めない（それは roadmap.md 側）。
...
4. リポの `roadmap.md` で本プリセットを指し、スクリプト実パス・リポ URL を記載する
```
...
## Task 4: roadmap.md を新設（このリポの issue 管理バインディング）
...
git commit -m "feat(plan): add roadmap.md companion for issue management binding
...
設計 §3 手順4。規約散文を roadmap.md へ移したので、description は依存構造グラフとしての最小事実に絞る。
...
description: オープン issue の依存関係と着手可能順序。issue 本体が一次情報、本ファイルは依存構造のみ管理。issue
  管理規約・同期手段は sibling roadmap.md と pfd-ops スキルの references/github-issues-backend.md 参照
```
...
- **作業項目の一次情報と同期手段**: `roadmap.pfdsl` とその sibling `roadmap.md` に従う
...
2. **新規作業の受け入れ**: 作業項目を起票（手段は roadmap.md）→ 依存グラフに1チェーン追加 → 並列性・接点・合流点を確定させてから着手する
...
2. **実行**: 作業項目の一次情報は roadmap.md が指すバックエンド。ブランチを切って作業する（main 直コミットしない）。PFD の読み書きは pfdsl スキルの品質ガイドに従う。まとまった執筆・実装は subagent に委譲し、本体は指示と評価に専念する
3. **反映 — 終端ゲート（全項目を明示的に確認。「該当なし」も判断として記録）**:
   ...
   - [ ] 論理単位でコミットした
   - [ ] 変更束を PR にまとめた
   - [ ] companion（roadmap.md 等）が定義するリポ固有の追加ゲート項目を確認した
...
新 SKILL.md だけを読み、「GitHub Issue を使わず roadmap.pfdsl だけで運用する架空リポ（roadmap.md なし）」が1サイクル回せるか机上で確認する。確認観点:
...
- L4 リポ固有 → .pfdsl の sibling .md companion（ecosystem.md / roadmap.md）
...
- L3 プリセット → Task 3 ✓
- L4 手続き知 → Task 2（ecosystem.md）+ Task 4（roadmap.md）✓
