...
v0.0.21 からの主な変更点（v0.0.22）：`type: roadmap` のファイルで、body の edge に現れる artifact id が front matter に宣言を持たない場合を検出する V035 を追加した（§15.17、#1125）。宣言ブロックだけを削除して edge 行を残すと、label も status も criteria も持たない「幽霊ノード」がグラフに残り、既存の W005（produced artifact のみ走査）はこれを素通りさせる。V035 は strict mode の有無に関わらず常に error（`ctx.strictly(...)` を経由しない）。**破壊的変更**を含む — この形を持つ既存の roadmap ファイルは、非 strict の `check` でも新たに error になる。
...
* §16 エラー表脇の型一覧（76行目付近）を更新
* §15.16 / §15.2 の `runtime-pipeline` 参照を `pipeline` に置き換え
* `packages/core/src/types/frontmatter.ts` の `PFD_TYPE_VALUES` を更新
* `packages/cli/src/index.ts` のエラー文言・ヘルプ文言を更新

v0.0.19 からの主な変更点（v0.0.20）：`>>` と `>>?` の意味論を肯定形で規定し、W008 を追加した（#938）。従来 `>>?` について規範的に書かれていたのは免除条項（境界照合の対象外・rank 非算入・V002 の1入力充足等）だけで、肯定的な内容はすべて非規範の助言だった。その配分のため、関係の実体を説明できない edge ほど `>>?` を選ぶほうが検査を通りやすいという勾配が生じていた。**破壊的変更**を含む — 通常入力を持たない Process は strict mode で新たに error になる（W008）。これは既存要求の一般化ではなく新しい制約である。従来の §15.2 は「通常入力がゼロでフィードバック入力のみを持つ Process も1入力を満たす」と明示的に**許容**しており、subflow 展開プロセスへの要求（§2.9.3 への forward reference）は対応する診断コードを持たない助言文だった。

* §9.1 に `>>` の実行時意味論を明記（宣言上の依存関係であり、実行のたびの存在を保証しない。生成頻度の差は `>>?` でなく artifact の分割で表す。`status:` は §15.16 により roadmap 種別でしか使えないため、この目的の手段として挙げない）
* §9.2 に、補助入力が主駆動源にならないこと・適用条件は §15.3 が定めることを追記
* §15.3 を免除条項の列挙から適用条件（肯定形）の規定へ書き直し、「subflow 境界配線のコストを避けること」が適用条件でない旨と、その診断に先立つ下流判定を規範化。あわせて、この診断が機械検査に落ちない理由（Primary Graph の到達可能性は「配線を怠った非連結」と「正当な別系統からの補助入力」を区別できない）を明記
* §15.2 に W008 を規範として定義
* §16 エラー表に W008 の行を追加
* §17.6 の条件分岐の例に `spec >> impl` を補い、その例が単独ファイルとして通常入力ゼロの形にならないようにした
* `packages/core/src/rules/completeness.ts` に `feedbackOnlyProcess`（W008）を追加し `validator.ts` へ配線
