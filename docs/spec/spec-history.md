...
v0.0.19 からの主な変更点（v0.0.20）：`>>` と `>>?` の意味論を肯定形で規定し、それに対応する W008 / W009 を追加した（#938）。従来 `>>?` について規範的に書かれていたのは免除条項（境界照合の対象外・rank 非算入・V002 の1入力充足等）だけで、肯定的な内容はすべて非規範の助言だった。その配分のため、関係の実体を説明できない edge ほど `>>?` を選ぶほうが検査を通りやすいという勾配が生じていた。**破壊的変更**を含む — 通常入力を持たない Process は strict mode で新たに error になる（W008）。

* §9.1 に `>>` の実行時意味論を明記（宣言上の依存関係であり、実行のたびの存在を保証しない。生成頻度の差は `>>?` でなく status または artifact の分割で表す）
* §9.2 に、補助入力が主駆動源にならないこと・適用条件は §15.3 が定めることを追記
* §15.3 を免除条項の列挙から適用条件（肯定形）の規定へ書き直し、「subflow 境界配線のコストを避けること」が適用条件でない旨と、その診断に先立つ下流判定を規範化
* §15.2 に W008 を規範として定義（subflow 展開プロセスへの既存要求を全 Process へ一般化）
* §16 エラー表に W008 / W009 の行を追加
* `packages/core/src/rules/completeness.ts` に `feedbackOnlyProcess`（W008）、`rules/cycles.ts` に `subflowFeedbackDebt`（W009）を追加し `validator.ts` へ配線
