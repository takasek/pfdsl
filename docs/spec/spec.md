...
* 展開プロセスの通常入力エッジ（`>>`）が指す artifact の **ID 集合**（マップ適用後）と、子フローの **open input artifact**（生成元プロセスなし）の **ID 集合**は一致しなければならない（全単射・集合一致）。フィードバック入力（`>>?`）は照合対象外
* 展開プロセスの出力エッジ（`->`）が指す artifact の **ID 集合**（マップ適用後）と、子フローの **terminal artifact**（消費先プロセスなし）の **ID 集合**は一致しなければならない（全単射・集合一致）
