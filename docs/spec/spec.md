...
* `pfdsl ready` / `pfdsl status-set` は `type: roadmap` 以外の値を明示指定したファイルに対して error を出力する。省略時は `roadmap` として扱い実行を許可するが、warning (W006、§15.14) を出す
...
* 省略時は種別を問わない操作（check / fmt / graph 等）を実行する。`pfdsl ready` / `pfdsl status-set` は例外的に、省略時は `roadmap` として扱い実行を許可するが warning を出す (W006)。`roadmap` 以外の値を明示指定した場合は error
...
* `pfdsl ready` / `pfdsl status-set` が `type:` 省略ファイルを `roadmap` として扱う（§15.14）: warning (W006)
