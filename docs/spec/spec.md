...
* `pfdsl ready` / `pfdsl status-set` / `pfdsl audit-sync`（roadmap引数）は `type: roadmap` 以外の値を明示指定したファイルに対して error を出力する。省略時は `roadmap` として扱い実行を許可するが、warning (W006、§15.14) を出す
...
* **audit-terminal**（終端監査の対象）— 通常入力で消費されない成果物。フィードバック消費は無視するため、通常入力・フィードバック入力のいずれでも消費されない成果物に加え、フィードバック入力でのみ消費される成果物も含む。終端監査（`pfdsl check --audit`）はこれらを「消費者が疑わしい終端」として列挙する。
...
strict mode では、フィードバック元 A が Primary Graph 上で対象プロセス P から**順方向**（P の出力方向へ有向辺を辿る）に到達可能であることを追加制約として検査してよい。到達不能な場合は error (V011)。
...
**artifact 側との非対称** — frontmatter `artifact:` セクションで宣言され本文エッジに現れない artifact は、V020（プロセス側の孤立宣言 error）と異なり error・warning のいずれも報告しない。プロセスは入出力を持つ変換であり本文で使われなければ残骸だが、artifact は正当な孤立成果物宣言（source でも terminal でもない参照予定のプレースホルダ等）と typo を機械的に区別できないため、対称の検査を課さない。
...
* 省略時は種別を問わない操作（check / fmt / graph 等）を実行する。`pfdsl ready` / `pfdsl status-set` / `pfdsl audit-sync`（roadmap引数）は例外的に、省略時は `roadmap` として扱い実行を許可するが warning を出す (W006)。`roadmap` 以外の値を明示指定した場合は error
...
| V002 | error | §15.2 | Process に入力が無い |
| V003 | error | §15.2 | Process に出力が無い |
...
| V011 | error | §15.3 | strict mode: feedback artifact が Process から到達不能 |
...
| V020 | error | §15.10 | フロントマター宣言 Process が edge に不参加（孤立宣言プロセス） |
...
| W006 | warning | §15.14 | ready-gate 文脈（ready / status-set / audit-sync）で `type:` 省略ファイルを roadmap として扱う |
...
* 重複 edge: 無視してよい（§15.4 参照。checker は診断を発行せず黙って無視する）
