...
**再利用制約** — 境界 ID 協定により子フローの境界 ID は親エッジ ID と一致する必要があるため、境界 ID の異なる複数の親プロセスで同一子フローを共有（再利用）することはできない。本仕様は清水 PFD の「1 親プロセス : 1 子フロー」分解を前提とする。
...
* 展開プロセスの通常入力エッジ（`>>`）が指す artifact の **ID 集合**と、子フローの **open input artifact**（生成元プロセスなし）の **ID 集合**は一致しなければならない（全単射・集合一致）。フィードバック入力（`>>?`）は照合対象外
* 展開プロセスの出力エッジ（`->`）が指す artifact の **ID 集合**と、子フローの **terminal artifact**（消費先プロセスなし）の **ID 集合**は一致しなければならない（全単射・集合一致）
* 境界 ID 協定・メタデータ権威・再利用制約の詳細は §2.9.3 参照
* `subflow:` の値がファイルパスとして存在しない場合は error
* 循環 subflow（自己参照および3ホップ以上の多段循環を含む、subflow グラフ全体における任意の循環）は error
...
* §2.9.3 subflow 再利用禁止を設計判断として明文化（「1 親プロセス : 1 子フロー」前提）
