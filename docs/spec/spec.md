...
**location** — 成果物の実体ファイル・リソース、またはプロセスの追跡・実行文脈（issue/PR 等）へのポインタ（パス、glob、または URL）。スカラー文字列または文字列配列で指定する（後方互換: スカラーは単一要素配列と等価）。相対パスの基準は含む `.pfdsl` ファイルの位置（`basePath:` 参照）。グラフ意味論に影響しない。1 ノードにつき 0 または 1 個。§15.8 参照。
