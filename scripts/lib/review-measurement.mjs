 ...
 * text. Measured on origin/main: 82 records by line, 60 by git's reading, and
 * all 22 of the difference were genuine records rather than the prose #726
 * reported. Walking paragraphs keeps those while still ending the region at
 * the prose that made a commit explaining the mechanism fail its own gate.
 ...
 * commit and then per trailer line before parsing — `parseMeasurementTrailer`
 * reads only the first match in whatever it is handed. Both callers — the
 * merged-history audit and the pre-PR gate — need the same cut, and the way it
 * is made is the sort of thing that drifts silently when it lives in two
 * places. The per-commit cut is what keeps one message's trailing prose from
 * ending the region of the message that follows it, so both callers separate
 * their commits with RECORD_SEP.
...
export function parseMeasurementRecords(text) {
	return text
		...
		.filter((line) => line.startsWith("Review-Measurement:"))
