// Checks production and delivery of declared hand-written bundled artifacts.
// Manifest membership alone does not require diagram participation.
// Native V001 owns producer uniqueness; this check requires existence.
//
 ...
 * workflow production only makes sense for an artifact workflow.pfdsl
...
export function findUnwiredSkills({
	...
	const produced = edgeMembers(workflowEdges, { kind: "output" });
	...
	for (const [id, { meta, declaredIn }] of universe) {
		...
		const missing = [];
		if (declaredIn === "workflow" && !produced.has(id))
			missing.push("workflow producer");
