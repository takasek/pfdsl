...
describe("usesPublishedCli", () => {
	it("flags npx against the published CLI", () => {
		assert.equal(
			usesPublishedCli("npx @pfdsl/cli check .pfdsl/roadmap.pfdsl"),
			true,
		);
	});
	...
	it("flags it behind npx flags, in a compound command, and via pnpm dlx", () => {
		...
		assert.equal(
			usesPublishedCli("pnpm -r build && npx @pfdsl/cli check x.pfdsl"),
			true,
		);
	...
	it("ignores the local build, which is what this repo should be running", () => {
		assert.equal(
			usesPublishedCli("node packages/cli/dist/cli.js check x.pfdsl"),
			false,
		);
	});
...
describe("usesBodyDroppingView", () => {
	...
	it("allows it once --json asks for the body explicitly", () => {
		assert.equal(
			usesBodyDroppingView("gh issue view 650 --json body,comments"),
			false,
		);
		assert.equal(
			usesBodyDroppingView("gh issue view 650 --comments --json comments"),
			false,
		);
	});
	...
	it("ignores other gh groups and verbs", () => {
		assert.equal(
			usesBodyDroppingView("gh issue list --label flow:managed"),
			false,
		);
	...
	it("does not flag the flag inside a quoted string", () => {
		assert.equal(
			usesBodyDroppingView('echo "gh issue view 650 --comments"'),
			false,
		);
	});
	...
	it("flags it behind a gh global flag, where the group is not the second token", () => {
		assert.equal(
			usesBodyDroppingView("gh -R owner/repo issue view 650 --comments"),
			true,
		);
		assert.equal(
			usesBodyDroppingView("gh --repo owner/repo pr view 664 --comments"),
			true,
		);
	});
...
describe("evaluateCommandUsageGuard", () => {
	...
	it("allows an ordinary command", () => {
		assert.equal(
			evaluateCommandUsageGuard(payload({ command: "git status" })).decision,
			"allow",
		);
	});
	...
	it("asks before running the published CLI, naming the local build", () => {
		const result = evaluateCommandUsageGuard(
			payload({ command: "npx @pfdsl/cli check x.pfdsl" }),
		);
	...
	it("denies a body-dropping view, naming the flag that keeps the body", () => {
		const result = evaluateCommandUsageGuard(
			payload({ command: "gh issue view 650 --comments" }),
		);
	...
	it("reports the deny when a command trips both rules", () => {
		const result = evaluateCommandUsageGuard(
			payload({
				command: "gh issue view 650 --comments && npx @pfdsl/cli check x.pfdsl",
			}),
		);
		assert.equal(result.decision, "deny");
	});
...
describe("runCommandUsageGuard", () => {
	...
	it("produces no output for an ordinary command", () => {
		const { shouldOutput, output } = runCommandUsageGuard(
	...
	it("silently allows malformed stdin JSON", () => {
		assert.deepEqual(runCommandUsageGuard("not json{{{"), {
			shouldOutput: false,
		});
	});
});
