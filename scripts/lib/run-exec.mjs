 ...
 *
 * `git ls-files` without `-z` escapes any path containing a byte outside ASCII
 * and wraps it in quotes, so the caller gets `"\346\274\242.md"` where the file
...
export function gitLsFiles(patterns, { cwd, exec = git } = {}) {
	return exec(["ls-files", "-z", ...patterns], { cwd })
		.split("\0")
		.filter(Boolean);
}
