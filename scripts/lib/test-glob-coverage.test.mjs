...
import { extractTestGlobs, matchesGlob, findUnrunTestFiles } from "./test-glob-coverage.mjs";

const root = resolve(dirname(fileURLToPath(import.meta.url)), "../..");

describe("extractTestGlobs", () => {
	it("extracts the quoted globs of a `node --test` invocation", () => {
		const source = '\tnode --test "scripts/lib/*.test.mjs" "scripts/pfdsl/lib/*.test.mjs"\n';
		assert.deepEqual(extractTestGlobs(source), ["scripts/lib/*.test.mjs", "scripts/pfdsl/lib/*.test.mjs"]);
	});
	...
	it("collects globs from several `node --test` lines in one file", () => {
		const source = ['node --test "a/*.test.mjs"', "other command", 'node --test "b/*.test.mjs" "c/*.test.mjs"'].join(
			"\n",
		);
		assert.deepEqual(extractTestGlobs(source), ["a/*.test.mjs", "b/*.test.mjs", "c/*.test.mjs"]);
	});
	...
	it("ignores a `node --test` line whose arguments are unquoted", () => {
		...
		assert.deepEqual(extractTestGlobs("node --test scripts/lib/*.test.mjs\n"), []);
	});

	it("returns an empty array when no `node --test` line is present", () => {
		assert.deepEqual(extractTestGlobs('node scripts/check-script-imports.mjs\nrun: pnpm -r test "x"\n'), []);
	});
});

describe("matchesGlob", () => {
	it("matches a file in the globbed directory", () => {
		assert.equal(matchesGlob("scripts/lib/gate-check.test.mjs", "scripts/lib/*.test.mjs"), true);
	});

	it("does not let `*` span a directory separator", () => {
		assert.equal(matchesGlob("scripts/lib/nested/deep.test.mjs", "scripts/lib/*.test.mjs"), false);
	});

	it("does not match a file in a sibling directory", () => {
		assert.equal(matchesGlob("scripts/review-measurement.test.mjs", "scripts/lib/*.test.mjs"), false);
	});
...
describe("findUnrunTestFiles", () => {
	...
	it("returns the files no glob matches", () => {
		...
		assert.deepEqual(findUnrunTestFiles(files, ["scripts/lib/*.test.mjs"]), ["scripts/orphan.test.mjs"]);
	});
...
describe("the repository's own test globs", () => {
	...
	function trackedTestFiles() {
		const stdout = execFileSync("git", ["ls-files", "*.test.mjs"], { cwd: root, encoding: "utf-8" });
		return stdout.split("\n").filter(Boolean);
	}
	...
	it("`make test` runs every tracked *.test.mjs file", () => {
		const unrun = findUnrunTestFiles(trackedTestFiles(), extractTestGlobs(makefile));
		assert.deepEqual(unrun, [], `Makefile's node --test globs never reach: ${unrun.join(", ")}`);
	});

	it("CI runs every tracked *.test.mjs file", () => {
		const unrun = findUnrunTestFiles(trackedTestFiles(), extractTestGlobs(workflow));
		assert.deepEqual(unrun, [], `test.yml's node --test globs never reach: ${unrun.join(", ")}`);
	});
