...
describe("isCliEntrypoint", () => {
	...
	it("recognises the entrypoint when the invocation path crosses a symlink", () => {
		assert.equal(isCliEntrypoint("file:///private/tmp/scripts/s.mjs", "/tmp/scripts/s.mjs", { realpath }), true);
	});

	it("recognises the entrypoint when no symlink is involved", () => {
		assert.equal(isCliEntrypoint("file:///opt/scripts/s.mjs", "/opt/scripts/s.mjs", { realpath }), true);
	});

	it("rejects a different module imported by the entrypoint", () => {
		assert.equal(isCliEntrypoint("file:///opt/scripts/lib.mjs", "/opt/scripts/s.mjs", { realpath }), false);
	});

	it("rejects an absent argv[1]", () => {
		assert.equal(isCliEntrypoint("file:///opt/scripts/s.mjs", undefined, { realpath }), false);
	});
	...
	it("falls back to the verbatim comparison when the path cannot be resolved", () => {
		...
		assert.equal(isCliEntrypoint("file:///opt/scripts/s.mjs", "/opt/scripts/s.mjs", { realpath: throwing }), true);
	});
	...
	it("recognises a path whose characters need percent-encoding in a URL", () => {
		assert.equal(isCliEntrypoint("file:///opt/my%20scripts/s.mjs", "/opt/my scripts/s.mjs", { realpath }), true);
	});
});
