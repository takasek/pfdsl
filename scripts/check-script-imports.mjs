 ...
 * packages/**, so `pnpm -r typecheck` doesn't see it, and Biome — which does
 * lint it — never checks that an import's target exists, so nothing else
 * catches a sibling script's import specifier going stale when
