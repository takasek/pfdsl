...
- **区分 i — PFD 固有の意味論・活用法。** 記法、PFD 種別の問診、後ろ向き導出による執筆、着手可能性と終端監査の契約（監査対象の PFD と作業項目がどの状態なら着手可能・完了と判定するか）、status 更新、図の監査観点。
...
- **区分 iii — 有用だが利用者の環境に任せるもの。** git 衛生、委譲規律、設計記録の書式と再承認、レビュー menu、commit 粒度、1回の監査の実行管理（実行 ID、cutoff、情報源集合の凍結、並行実行や引継ぎの checkpoint）。配布しない。

監査に関わる区分 i・ii の項目が定めるのは、監査の対象について何を判定するかである。この意味で、着手可能性・終端監査の契約は区分 i に、知識成果物のライフサイクル監査の項目は区分 ii に入る。
その判定を1回の実行としてどう凍結し、記録し、引き継ぐかは PFD の意味論からは導けず、実行環境・並行性・証拠の可視性によって決まるので区分 iii とする。
監査の観点を配布していても、その実行管理は採用リポの binding に置く。
...
この区分を定めた経緯は上流リポの ADR-0039、監査の対象と実行管理を分けた経緯は ADR-0040 にある（`ADR-\d+` の解決範囲は下の「配布物中の ADR 参照の解決」が定める）。経緯は判定に要らない。
