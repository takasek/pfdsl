...
知識成果物ライフサイクル監査: (採用するなら「採用する。対象: …」と書く。書かなければ監査しない)
...
採用する対策は、その操作の既存手順・入力・道具・テストへ反映する。先例を使う委譲、変更前後の報告、最終レビューは、採用リポの `.pfdsl/bindings/pfd-ops.md`「ワークサイクルの追加手順」が持つ（その節を置かないリポでは pfd-ops の運用契約と終端ゲートだけが効く）。
毎サイクルの retro は同 reference の定期監査と手順5に従い、通知の有無で省略しない。

## 1 回の実行契約

(1回の監査へ入れる層と情報源、cutoff、coverage の扱い、handoff 前の checkpoint をリポの運用に合わせて記入する。記入しない場合、pfd-retro は適用単位の判断だけで実行する。)
