...
最初の実装コミットより前に決定を変更する場合だけ、同じコメントを編集してよい。編集後は `改訂履歴:` の `- なし` を、旧決定、新決定、変更理由、再承認 URL を持つ行へ置き換える。完全な Format 3 コメントを別コメントとして追加して既存記録を置換してはならない。複数の完全な形式3コメントがある場合は正本を推測せず fail-close とする。
