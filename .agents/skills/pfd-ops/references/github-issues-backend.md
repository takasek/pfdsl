...
- Node.js 24 以上
- `audit-issues-flow.mjs` を実行する環境では、対象リポの GitHub ホストへ認証済みの `gh` CLI（`gh auth status --hostname <host>` が成功する状態。自前の CI 等で `GH_TOKEN` を渡して `gh` を認証する方式も含む。同梱の workflow は監査を実行しない）
...
v0.0.26 からこの節を含む版の直前までの本 reference は、`gh` が無くても `GH_TOKEN` / `GITHUB_TOKEN` があれば監査を実行できると案内していた。この版から、支援する実行環境は認証済みの `gh` だけになる。
