<!-- DO NOT EDIT. Authoritative source: .claude/skills/pfd-ops/references/github-issues-backend.md. -->
...
対応方針の候補を2件以上列挙した issue は、実行主体が着手前（ブランチの初コミットより前）に投稿する完全な Format 3 設計選択記録があるまで設計未確定として扱う。Format 3 は `設計記録形式: 3` を宣言し、work-cycle 手順1 適用点1 が定義する `決定:`、`理由:`、`案の処分:`、`前提検査 Pn:`、`改訂履歴:` をこの順で持つ。**投稿先は当該 issue のコメントに限る** で、前後は記録の投稿時刻または最終編集時刻の遅い方と初コミットの authorDate を機械照合する。
...
最初の実装コミットより前に決定を変更する場合だけ、同じコメントを編集してよい。再承認参照は、同じ issue のコメントを指す canonical comment URL または `対話 ` に続く ISO8601 UTC 時刻の2形式だけを使い、編集後は `改訂履歴:` の `- なし` をその形式の再承認参照を持つ改訂行へ置き換える。完全な Format 3 コメントを別コメントとして追加して既存記録を置換してはならない。複数の完全な形式3コメントがある場合は正本を推測せず fail-close とする。
