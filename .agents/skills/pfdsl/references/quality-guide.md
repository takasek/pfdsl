...
- **実装が実在するならエッジは実装で裏取り**: runtime-pipeline 等でプロセスに対応するコードが既にあるなら、対応関数のシグネチャ（引数・戻り値）を読んでから `>>`/`->` を引く。憶測エッジ禁止 — 名前からの推測は入力の取り違え（source text か parsed 構造か等）を生む
