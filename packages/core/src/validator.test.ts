...
describe("validate", () => {
	...
	describe("schedule: field (#220)", () => {
		it("valid process schedule: no diagnostics", () => {
			const fm: Frontmatter = {
				process: {
					P: {
						schedule: {
							workVolume: 3.0,
							reworkRatio: 0.3,
							resources: "Dev:1;QA:0.5",
							startCondition: "\\complete(A)",
							milestone: "M1",
						},
					},
				},
				artifact: { A: { criteria: "x" }, B: { criteria: "y" } },
			};
			expect(codes("A >> P -> B", fm)).not.toContain("V029");
			expect(codes("A >> P -> B", fm)).not.toContain("V030");
			expect(codes("A >> P -> B", fm)).not.toContain("V031");
		});

		it("valid artifact schedule: no diagnostics", () => {
			const fm: Frontmatter = {
				artifact: {
					A: {
						criteria: "x",
						schedule: { availableTime: 0.0, maxRevision: 3 },
					},
					B: { criteria: "y" },
				},
			};
			const c = codes("A >> P -> B", fm);
			expect(c).not.toContain("V029");
			expect(c).not.toContain("V030");
			expect(c).not.toContain("V031");
		});

		describe("V029: schedule must be a mapping", () => {
			it("errors when schedule is a scalar on a process", () => {
				const fm: Frontmatter = { process: { P: { schedule: 3 } } };
				expect(codes("A >> P -> B", fm)).toContain("V029");
			});

			it("errors when schedule is an array on an artifact", () => {
				const fm: Frontmatter = {
					artifact: { A: { schedule: [1, 2] } },
				};
				expect(codes("A >> P -> B", fm)).toContain("V029");
			});

			it("V029 severity is error", () => {
				const fm: Frontmatter = { process: { P: { schedule: "x" } } };
				const v029 = diagnose("A >> P -> B", fm).find((d) => d.code === "V029");
				expect(v029?.severity).toBe("error");
			});
		});

		describe("V030: unknown schedule field for the node type", () => {
			it("errors on an unknown key", () => {
				const fm: Frontmatter = {
					process: { P: { schedule: { bogus: 1 } } },
				};
				expect(codes("A >> P -> B", fm)).toContain("V030");
			});

			it("errors when an artifact-only field is used on a process", () => {
				const fm: Frontmatter = {
					process: { P: { schedule: { availableTime: 0 } } },
				};
				expect(codes("A >> P -> B", fm)).toContain("V030");
			});

			it("errors when a process-only field is used on an artifact", () => {
				const fm: Frontmatter = {
					artifact: { A: { schedule: { workVolume: 1 } } },
				};
				expect(codes("A >> P -> B", fm)).toContain("V030");
			});
		});

		describe("V031: invalid value for a known schedule field", () => {
			it("errors when workVolume is negative", () => {
				const fm: Frontmatter = {
					process: { P: { schedule: { workVolume: -1 } } },
				};
				expect(codes("A >> P -> B", fm)).toContain("V031");
			});

			it("errors when reworkRatio is above 1", () => {
				const fm: Frontmatter = {
					process: { P: { schedule: { reworkRatio: 1.5 } } },
				};
				expect(codes("A >> P -> B", fm)).toContain("V031");
			});

			it("errors when resources is not a string", () => {
				const fm: Frontmatter = {
					process: { P: { schedule: { resources: 5 } } },
				};
				expect(codes("A >> P -> B", fm)).toContain("V031");
			});

			it("errors when maxRevision is not an integer", () => {
				const fm: Frontmatter = {
					artifact: { A: { schedule: { maxRevision: 2.5 } } },
				};
				expect(codes("A >> P -> B", fm)).toContain("V031");
			});

			it("accepts reworkRatio at boundary values 0 and 1", () => {
				const fm0: Frontmatter = {
					process: { P: { schedule: { reworkRatio: 0 } } },
				};
				const fm1: Frontmatter = {
					process: { P: { schedule: { reworkRatio: 1 } } },
				};
				expect(codes("A >> P -> B", fm0)).not.toContain("V031");
				expect(codes("A >> P -> B", fm1)).not.toContain("V031");
			});
		});
	});
});
