...
対象パターンごとに次の9観点を表へ記録する。
初回判定は「妥当」「finding」「保留」のいずれかとし、finding を修正して同じ reviewer が再検証した欄は「修正後妥当」または「finding」で終状態を記録する。
「修正後妥当」は初回 finding が存在した履歴と修正後に解消した状態を1つの値で保持し、「妥当」への上書きで初回 finding を消さない。
