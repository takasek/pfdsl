...
description: |
  Use before a CLI release, after bundle changes in Claude Code, or when
  explicitly asked to audit the bundle for upstream-only assumptions.
  Simulates an adopting repo's reader with sandboxed subagents given only the
  bundle, fixes what makes them stall, and records the reviewed commit that
  `make release` gates on. During release preparation, this handles unreviewed
  files reported by release-status or a refusal from `make release` because
  the distributed prompts changed. Perspectives live in docs/distribution-review.md.
---
...
Claude Code では配布層の変更後にも、既存の体験レビューの規約に従って実行する。
Codex では通常の配布文言の編集ごとには起動せず、内容に合った差分レビューと生成・構文検査を行う。
Codex で担当する利用側シナリオの模擬実行は、公開前、またはその検証を明示的に依頼されたときに行う。
...
| hash | **更新する** | **更新しない** |
| 起動 | 公開前に `make release` が要求する / Claude Code で配布層を変更した後 / 人が差分監査を頼んだとき | 人が全文監査を頼んだときだけ |
