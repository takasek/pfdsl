...
このリポで過去に検出したパターンは `.pfdsl/bindings/<このスキル名>.md` が持つ。**その全読を消費方法にしない。** 蓄積される成果物なので、件数が増えれば全読は毎サイクル効かなくなる — binding が引き方（索引・検索コマンド等）を定めていればそれに従い、定めていなければ引き方を持つ形へ改めることを D 層「スケール監査」の finding として扱う。
引いた結果が0件でなくても、それで網羅したとは限らない。**タグや見出しは書いた人が予期できた軸しか答えない**ので、binding が本文検索の手段を持つならサイクルの具体語（変更したファイル名・触ったフラグ・エラー文言）でも引き、両者の差を見る。0件だけを立ち止まる契機にすると、N件返った回が素通りする。
