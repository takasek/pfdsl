...
C・D 層で過去に検出済みのパターンは `.pfdsl/bindings/pfd-retro.md`「監査の追加パターン」節冒頭の索引から引く。全読はしない。
索引の各行は適用条件（どんな変更をしたサイクルで効くか）を持つので、今サイクルで実際にやったこと（触ったパス・委譲の有無・出した結論の形）と突き合わせ、**条件が成立する行だけ**を選んで、その見出し文字列で本文を検索し該当ブロックを読む。
「関係しそうか」という印象で選ばない — 印象で選ぶ形にすると、読み飛ばした範囲が毎回サイレントに決まり、その妥当性が記録に残らない。
...
| 監査の新パターン発見 | `.pfdsl/bindings/pfd-retro.md`。問いの構造・パターン種別・具体例を本文に書き、索引にも同じコミットで一行追加する（索引の行は適用条件を必ず持つ — 書けないパターンは「どんな回に効くか」が未言語化なので、本文側で詰めてから索引へ載せる）。**ここに載るのは標準カタログに無い新しい問いの構造だけ** — 既にカタログにある観点をこのリポで検出した実例は、bindings が指すインスタンス・カタログ（例: `.pfdsl/review-perspectives.md`）側に蓄積する。同じ「具体例」の語が両方に出るため、判定は「観点そのものが新しいか」で行う |
