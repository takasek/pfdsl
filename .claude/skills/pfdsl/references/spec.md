...
* **open input / terminal の判定**: open input artifact は生成元プロセス（`->`）を持たない artifact、terminal artifact は **`>>`（通常入力）でも `>>?`（フィードバック入力）でも消費されない** artifact とする。フィードバックのみで消費される artifact は子フロー内部のループ要素であり terminal ではない（境界に出さない）
...
* `pfdsl ready` は `type: roadmap` のみ実行を許可する。`type: workflow` / `type: runtime-pipeline` を指定したファイルに対しては error を出力する
...
* 展開プロセスの通常入力エッジ（`>>`）が指す artifact の **ID 集合**（マップ適用後）と、子フローの **open input artifact**（生成元プロセス `->` なし）の **ID 集合**は一致しなければならない（全単射・集合一致）。展開プロセスのフィードバック入力（`>>?`）は照合対象外
...
* 省略時は種別を問わない操作を実行する。`pfdsl ready` は `type: roadmap` のみ許可する
...
* フロントマター宣言の有無にかかわらず、edge の `->` で生成される全 artifact を対象とする
...
* §2.10 `type:` フィールドを追加（roadmap | workflow | runtime-pipeline）
  * 列挙外の値は V031 error
  * `pfdsl ready` は `type: roadmap` のみ許可
