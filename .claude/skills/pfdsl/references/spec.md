...
**境界 ID 協定** — ファイルローカル名前空間を維持したまま親子フローの整合を機械検証するため、境界 artifact にかぎり親子で同一 ID を共有する契約とする。
...
* **キー単位マージ**: 同一キーはローカル定義が prevail。衝突しないキーは union される（ブロック丸ごと置換ではない）
* **ローカル prevail**: ローカル定義は常に全プリセットに勝つ
* **複数 extends**: 配列指定の場合、同一キーが複数プリセットに存在すれば配列の末尾が勝つ（後勝ち）。ローカルは配列全体に勝つ
* **多段 extends**: プリセットファイル自身も `extends:` を持てる。解決は深さ優先・nearest-wins。`A extends B extends C` の場合、同一キーでは A > B > C の優先順
* **循環禁止**: `A extends B, B extends A` のような循環 extends は error
...
解決結果: `done` は青（`#2196F3`）、`wip` はプリセットの黄（`#FFC107`）。キー単位マージのため `wip` はローカル未定義でもプリセット値が有効となる。

`tag` は §2.7.4 で導入された tag 定義ブロックであり、extends 共有対象に含まれる。`tag` の各タグ ID もキー単位マージ対象であるため、ローカルで特定タグの style のみ上書きしつつプリセットの他タグ定義を引き継ぐことができる。
...
* `artifact:` / `process:` キーを含む場合は error（生成物定義の共有は禁止）
...
`subflow:` を持つ Process（展開プロセス）に対し、checker は以下の境界整合を検証する。

* 子フローの **open input artifact**（生成元プロセスなし）は、展開プロセスの入力エッジが指す artifact と同一 ID でなければならない
* 子フローの **terminal artifact**（消費先プロセスなし）は、展開プロセスの出力エッジが指す artifact と同一 ID でなければならない
* 境界 ID 協定の詳細は §2.9 参照
...
5. **継承解決順**: §2.9.4 に従う（キー単位マージ・nearest-wins・ローカル prevail）
...
* §2.9.4 extends 継承解決を規定（対象は statusStyles / `tag`（§2.7.4）/ group のみ。キー単位マージ・nearest-wins・ローカル prevail）
