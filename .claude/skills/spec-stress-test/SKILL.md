...
本スキルは2フェーズでそれを検出する。フェーズ1（ADR-0020）が**規則の穴**を、フェーズ2（agent プローブ）が**読者の躓き**を検出する — 検出対象が異なるため代替にならず、大きな変更では両方やる。小さな変更（単一制約節の追記）はフェーズ1のみでよい。
...
- CLI が実効値（深マージ後の解決結果等）を一切露出せず check の成否だけでは正解確定できない場合、対象の checker 実装関数を直接呼ぶ検証スクリプトで spec のアルゴリズム記述を忠実再現し正解を確定する。この確定作業自体が「実装が spec の当該機能を呼び出していない／露出していない」ギャップの検出装置にもなる — オラクル設計の副産物として実装ギャップが見つかることがある
