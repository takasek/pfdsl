...
- **Claude 向け指示の置き場**: 配布スキルに同梱すべき規約（PR 本文規約等）は `references/` に置く（**bundle 上流リポ takasek/pfdsl でのみ可能** — 採用リポの `.claude/skills/` は skill sync で上書きされるため、届けたい指示が生まれたら上流へ issue/PR で還流を提案する）。project CLAUDE.md は採用リポ固有の非配布設定のみ、global CLAUDE.md は全リポ横断設定のみ。「採用リポの Claude にも届けたい」指示は references/ へ
