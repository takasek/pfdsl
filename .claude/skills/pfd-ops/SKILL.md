...
1. **着手判断**: 入力 artifact が全て done のプロセス = 着手可能。並列着手集合は `pfdsl status ready <roadmap.pfdsl> --best --json` で機械的に導出する（roadmap 全文 Read で手動判定しない。優先順位の議論より先にまず列挙）。**作業項目に着手する回は、この判断を下す前に下の「ワークサイクル」節へ入り、その4手順に従う** — 着手判断はその手順1の一部であり、単独では完結しない。どの起動口から来たか（`/pfd-cycle` か、bare な作業項目番号を渡されたか）は問わない
...
5. **成果物の門番（双方向、ADR-0018）**: 終端監査は両向きに行う
   ...
   - **版 artifact の criteria は版一覧への包含で書く**（門番の向きではなく、上項で起こすと決めた artifact の完了条件の書き方）: 「最新版を返す手段」（`npm show <pkg> version`・ストアの表示版など）で特定の版を主張する criteria は、最新版を指す1件を除いて文字どおり偽になる。版一覧を返す手段への包含（`npm view <pkg> versions に X が含まれる` の形）で書く。同じレジストリを引く呼び方でも最新1件しか返さないものがあるため、採る手段は名前でなく実際の戻り値を実行して確かめてから criteria に書く
...
**この節は特定の起動口に属さない** — `/pfd-cycle` から入った回も、bare な作業項目番号を渡された回も、同じ4手順に従う。
