...
対応方針の候補を2件以上列挙した issue は、**起票者本人**が行頭 `決定: 案N` の1行を本文またはコメントに書くまで設計未確定として扱う（work-cycle 手順1）。`N` は列挙した案のラベル（`A`・`2`・`A+B` 等、issue 内の呼称と揃える）。

- **確定の証拠はこの行の実在のみ。** 「本命はこちら」「順序を示唆している」等のヘッジ語は証拠にならない。判定を自然言語の読み取りに預けると、実行主体が自分に有利な充足宣言をしても外形上は区別がつかない
- **author が起票者と一致しない `決定:` 行は無効。** 実行主体が自分で投稿した決定行は自動的に対象外になる。ただし実行主体が起票者と同じ GitHub 認証情報で `gh` を実行するリポでは author 照合が判別子にならない — その環境ではこの検査は「別アカウントの agent が書いた決定行」だけを弾く
- 着手前の選択記録（前提・否定案・却下理由）は別物。こちらは実行主体が書き、**ブランチの初コミットより前に**投稿する。行頭 `前提:` / `否定案:` / `却下理由:` の3行を含め、列挙された各案の処分（採用・却下・保留）を明示する
