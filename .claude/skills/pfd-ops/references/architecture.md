...
**0段目 — PFDSL が提供すると決めた用途に入るか。** 昇格候補になるのは、PFD 固有の意味論・活用法（区分 i）か、提供すると決めた連携・運用プリセット（区分 ii: 作業項目バックエンドとの連携、知識成果物ライフサイクル監査）に属するルールだけである。
固有名詞を含まず全利用者に効くことは必要条件であって十分条件ではない。
git 衛生・委譲・設計記録の書式のように PFD 運用に固有の用途を持たない規律は、どれほど汎用でも昇格しない。
