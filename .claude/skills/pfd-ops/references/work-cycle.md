...
1. **選択**:
   ...
   - **適用点1 — 作業項目が列挙する案の集合を、設計空間とみなさない。** 起票者が挙げた案は入力の一部であり、網羅性は保証されていない。設計を確定させる前に次を行う。
     ...
     **記録先**: 前提の1文・否定案・却下理由は、そのサイクルの作業記録（PR 本文・作業項目のコメント等、後から読み返せる場所）に残す。頭の中だけで済ませない — 残さないと、後から「前提を疑ったか」を第三者が判定できず、原則が効いているかの点検自体ができなくなる。
