...
work-cycle 手順1 は、対応方針の候補が2件以上ある作業項目を「起票者本人の `決定: 案N` 行」があるまで設計未確定として扱い、その記法の定義をバックエンドに委ねている。ファイルベースでの定義は次のとおり。

- **置き場**: 当該項目の本文に行頭 `決定: 案N` の1行を書く。`N` は列挙した案のラベル（`A`・`2`・`A+B` 等、項目内の呼称と揃える）
- **確定の証拠はこの行の実在のみ。** 「本命はこちら」「順序を示唆している」等のヘッジ語は証拠にならない
- **author 照合の代替**: markdown ファイルには行ごとの author が無いため、GitHub Issues 版の「起票者本人か」は原理的に検査できない。判別子はその行を追加した**コミットの author** に置き換える（`git log -S'決定: ' -- <一次情報ファイル>` で追える）。実行主体が起票者と同じ git identity でコミットするリポでは author 照合が判別子にならない点は GitHub Issues 版と同じ

## 着手前の選択記録

work-cycle 手順1 の適用点1（前提・否定案・却下理由）は、実行主体が書く記録であり上の `決定:` 行とは別物。ファイルベースでは記録先自体がリポ内ファイルなので、「投稿」＝コミットになる。

- 行頭 `前提:` / `否定案:` / `却下理由:` の3行を当該項目に追記し、**それだけを単独のコミットにする**
