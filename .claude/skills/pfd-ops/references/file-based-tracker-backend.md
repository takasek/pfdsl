...
- **移行履歴**: 時刻による旧形式の互換性は GitHub Issues backend だけに適用する。ファイルベースの新規記録と改訂は常に binding が定める現行の書式を使い、旧形式を新たに書かない。既存の旧形式は書き換えず、改訂行を追加する場合からこの backend のコミット SHA 形を使う。書式そのものをこの reference は定めない。
- **改訂履歴の再承認参照**: このリポにはファイルベース backend の参照解決器が無いため、以下は機械判定の契約ではなく散文による運用契約である。ファイルベースでは、承認を記録したコミットの SHA を再承認参照に使い、行の値は `コミット` という接頭辞、半角スペース、40桁の16進 SHA の順にする。実行主体はまず当該項目の一次情報ファイルにある `承認履歴:` へ承認内容を追記し、その追記だけを単独の通常コミットにする。空コミットは作らない。その承認コミットが存在してから、次の単独コミットで当該項目の記録へ改訂行を追記し、`コミット <40桁SHA>` にその先行コミットの SHA を書く。改訂行の構造は binding が定める書式に従い、この reference が定めるのは再承認参照の値の形だけである。したがって参照先は記録本文を書く時点ですでに存在し、実行主体は発明した空コミットに頼らずこの形を作れる。GitHub のコメント URLや対話時刻は使わない。
