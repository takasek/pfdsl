...
5. **成果物の門番（双方向、ADR-0018）**: 終端監査は両向きに行う
   ...
   - **運用の道具を運用図に登録する基準**（上項の系）: hook・ガード・agent・スクリプト等の道具を運用図のノードとして登録するのは、(1) 運用プロトコル文書（本スキル本文・`references/` の手順・プロジェクト指示）が一般形で名指しして要求する機構か、(2) そのリポが外部へ配る成果物に含まれるもの、のいずれかに限る。個別の事故から生まれたリポ固有の防止機構と、そのリポの開発都合の道具は、実在していても登録しない — 運用図が運ぶのは「この運用が要求する構造」であり、道具の実装一覧ではない。両者を混ぜると図の更新義務が事故対応のたびに発生し、そのぶん図は実装台帳へ寄っていく。**基準は道具の種類ごとに立てない** — 種類ごとに違う基準を書くと、同じ「図に何を置くか」の問いに複数の答えが並び、種類の境界に落ちた道具をどちらで測るかが決まらなくなる
...
6. **知見の振り分け**: 実践・レビューで得た知見を記録先成果物へ振り分ける。**構造的事実**（新しいエッジ・成果物の生成方式が変わった等、図に描ける変化）は対応する `.pfdsl` 本体のノード・エッジ・description・criteria を更新する。**手続き散文**（グラフで運べない運用ルール・振り分け手続き自体）は sibling companion `.md` に書く。両方に該当する変更（新成果物の追加等）は両方を更新する
   - **散文として書く前に機械化を検討する**: 振り分け先が散文（companion・スキル本文・プロジェクト指示）になる知見は、追記の前にその内容が機械判定できるか（コマンド文字列・ファイルパス・リポジトリの状態から決まるか）を確認する。機械判定できるなら hook 化が散文に優先する — 散文は読まれて初めて効くが、hook は毎回必ず発火する。散文にとどめてよい例外は2つで、(a) 守り忘れたときのコストが警告を出すコスト（hook の実装・維持・誤発火の煩わしさ）を明確には上回らない、(b) 既存の検査機構（コミットフック・終端ゲート・CI）が既に同じ対象を検査しており hook が重複になる。どちらにも当たらないなら hook 化を先に済ませ、散文はその後で書く。hook 化した場合、散文は「hook のメッセージが全情報を運ぶなら書かない」「hook でカバーしきれない残りだけを書く」のどちらかにとどめる — hook へのポインタを機械的に書き足すだけでは、メッセージと重複する散文が残る
   - **hook の決定の選べる幅は event で決まる**: PreToolUse には実行主体へ届く advisory チャネルが無い（`hookSpecificOutput.additionalContext` を返せる event は他にあるが PreToolUse はそこに含まれず、exit 0 の stderr は実行主体に渡らない）。どの event が advisory を返せるかは harness の版で変わるので、時点の観測として扱い、規則を書く前にその版の実体を確かめる。したがって PreToolUse で機械化するルールの選択肢は `deny`（対処法をメッセージに書けば1回の retry で自己修復する。対処済みかがルール側から見えるものに限る）と `ask`（人間に判断を渡す。対処済みかが payload から見えないルールはこちら — deny にすると retry も同じく止まって抜け道が無くなる）の2つで、「既定は advisory」は PreToolUse では成立しない。advisory で足りるルールは PostToolUse に寄せる（書き込み後の lint・起票直後の登録促しがこの形）
