...
5. **成果物の門番（双方向、ADR-0018）**: 終端監査は両向きに行う
   ...
   - **運用の道具を運用図に登録する基準**（上項の系）: hook・ガード・agent・スクリプト等の道具を運用図のノードとして登録するのは、それが図の変換の**参加者**である場合に限る — 図にあるプロセスの出力として生まれるか、図にある成果物を生産・消費するもの。個別の事故への対処として足した実装や、そのリポの開発都合のための道具は、実在していても参加者ではないので登録しない。運用図が運ぶのは「この運用が要求する構造」であり、道具の実装一覧ではない。両者を混ぜると図の更新義務が事故対応のたびに発生し、そのぶん図は実装台帳へ寄っていく。**基準は道具の種類ごとに立てない** — 種類ごとに違う基準を書くと、同じ「図に何を置くか」の問いに複数の答えが並び、種類の境界に落ちた道具をどちらで測るかが決まらなくなる。**参加者性の代理（配布物に含まれるか・プロトコル文書が名指ししているか等）を基準そのものに据えない** — 代理は十分条件にすぎず、基準として閉じると、参加者でありながら代理を満たさない道具が機械的に落ちる
