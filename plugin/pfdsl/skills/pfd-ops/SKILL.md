...
5. **成果物の門番（双方向、ADR-0018）**: 終端監査は両向きに行う
   ...
   - **運用機構を運用図に登録する基準**（上項の系）: hook・ガード等の運用機構を運用図のノードとして登録するのは、運用プロトコル文書（本スキル本文・`references/` の手順・プロジェクト指示）が名指しで要求する一般的機構だけに限る。個別の事故から生まれたリポ固有の防止機構は、実在していても登録しない — 運用図が運ぶのは「この運用が要求する構造」であり、機構の実装一覧ではない。両者を混ぜると図の更新義務が事故対応のたびに発生し、そのぶん図は実装台帳へ寄っていく
   - 変換コンポーネントを追加・変更・削除する場合は、その変換を実際にモデル化している採用済み PFD の description・criteria・edge を更新する。**成果物の格納先・生成方式を変える場合も同じ扱いで、`location` を含めて同一 PR で更新する** — 格納先はその artifact の定義の一部なので、変えたまま図を放置すると図と実装が乖離する。「runtime-pipeline.pfdsl が存在しない = 該当なし」と即断しない — 別の PFD（多くの場合 `.pfdsl/workflow.pfdsl` の該当ノード・エッジ）が同じ変換を表現していないか確認してから N/A と記録する
