...
- **区分 iii — 有用だが利用者の環境に任せるもの。** git 衛生、委譲規律、設計記録の書式と再承認、レビュー menu、commit 粒度、監査や作業の1回の実行の管理（実行 ID、cutoff、情報源集合の凍結、並行実行や引継ぎの checkpoint）。配布しない。

区分 i・ii が定めるのは、監査の対象について何を判定するかである。この意味で、着手可能性・終端監査の契約は区分 i に、知識成果物のライフサイクル監査の項目は区分 ii に入る。
