...
設計選択記録は、実行主体が着手前に確定する記録である。その書式と再承認参照の検査は採用リポの binding が定める。ファイルベースでは記録先自体がリポ内ファイルなので、確定操作はコミットになる。
...
- **移行履歴**: 時刻による旧形式の互換性は GitHub Issues backend だけに適用する。ファイルベースの新規記録と改訂は常に binding が定める現行の書式を使い、旧形式を新たに書かない。既存の旧形式は書き換えず、改訂行を追加する場合からこの backend のコミット SHA 形を使う。書式そのものをこの reference は定めない — 採用リポの binding に書式が無ければ、記録を作る前にそれを用意する。
