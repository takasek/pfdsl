...
存在の有無を扱いたい場合に用いるのは §9.2 のフィードバック入力ではなく、`status:`（§2.7）による状態の表現、または頻度の異なる依存を別の Artifact へ分割することである。
...
subflow 展開プロセスは境界整合のため通常入力を1つ以上持つべきであり（§2.9.3）、W008 はその要求を全 Process へ一般化したものである。
...
処理系は、subflow 展開プロセスへ入る `>>?` のうちフィードバック元が当のプロセスの下流にないものを warning (W009) として報告する。
...
| W009 | warning | §15.3 | subflow 展開プロセスへ入る `>>?` のフィードバック元が、当のプロセスの下流にない |
