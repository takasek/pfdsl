...
構造 parser の placeholder 判定は再承認 cutoff の免除対象外なので、cutoff 前の記録でも雛形の空欄語が本文に残れば `template placeholder remains` で FAIL になる。再承認参照の空欄語もこの対象であり、判定が見る語の範囲は work-cycle 手順1 適用点1 の機械検査の記述が一次情報。
