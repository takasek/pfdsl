...
対応方針の候補を2件以上列挙した issue は、実行主体が着手前（ブランチの初コミットより前）に投稿する設計選択記録（`提案:` / `理由:` / `前提を外した対案:` / `対案を採らない理由:`。work-cycle 手順1 適用点1 が定義する）があるまで設計未確定として扱う。4行はこの順で、候補ごとの処分は `案の処分 N:` で明示する。実装しない決定では `実装しない:` も明示する。**投稿先は当該 issue のコメントに限る** で、前後は記録の投稿時刻（`createdAt`）と初コミットの authorDate を機械照合する。
...
**移行互換**: `2026-08-30T09:32:50Z` より前に作成されたコメントは、旧 `前提:` / `否定案:` / `却下理由:` 形式でも有効な設計選択記録として扱う。この時刻以降のコメントは新4行をすべて持つ場合だけ有効である。新旧の候補がある場合は新形式を優先する。旧形式の既存コメントを書き換えて移行しない。
