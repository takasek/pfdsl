...
work-cycle 手順1 の適用点1（`提案:` / `理由:` / `前提を外した対案:` / `対案を採らない理由:`）は、実行主体が着手前に確定する記録である。4行はこの順で、列挙した各案の処分は `案の処分 N:` で明示する。実装しない決定では `実装しない:` も明示する。ファイルベースでは記録先自体がリポ内ファイルなので、確定操作はコミットになる。
...
- **移行互換**: `2026-08-30T09:32:50Z` より前に作成された GitHub コメントだけは、旧 `前提:` / `否定案:` / `却下理由:` 形式のまま有効である。ファイルベースの新規記録は新4行を使い、旧形式を新たに書かない。
