...
- **順序の限界**: ファイルベースには外部サービスの server-side timestamp に対応する独立の時間窓は無い。代わりに `git log --reverse` 上で、承認コミット、改訂行を含む記録コミット、実装の初コミットがこの順に並ぶことを順序契約とする。この commit graph の順序が担保するのは参照先が記録本文より前にあることと記録が実装より前にあることだけで、承認の実在、本文の意味、各改訂行との対応は人間レビューの責務である。
