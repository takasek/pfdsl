...
- **委譲ブリーフの片肺 trap**: companion `.md` の記述更新を委譲するとき、同じ事実を述べる sibling `.pfdsl` のノード description をブリーフに含めないと、委譲先は指示された `.md` 側だけを直して戻る。
  ...
  具体例: `.svg` のドリフト検査機構を変えた回で、ブリーフは `.pfdsl/roadmap.md` と `.pfdsl/workflow.md` の該当記述を名指ししたが、同じ検査機構を述べる `runtime-pipeline.pfdsl` の `sample_previews.description` を含めていなかった（終端ゲートの「変換コンポーネント反映」項目で捕捉、#588）。
