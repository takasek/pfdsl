...
- **並行委譲の接合部**: 複数 subagent へ並行委譲した成果物同士の整合は、各委譲の受け入れ基準では検証されない。
  ...
  問いの形: 「接合する出力・操作は、成功時と失敗時の両方で、相手側の解釈・観測・状態遷移の契約を保っているか」。
  ...
  **同じ形は rebase で併合した、独立に正しい契約同士にも起きる。** Claude root の bundle manifest は全ファイルを書き終えた後に記録する必要があり、Codex migration の cleanup は legacy な Claude-root files を消す必要がある。各変更を単独に通しても、manifest を cleanup より先に書く接合順では、後に消す file まで hash に記録してしまう。
  ...
  対策: 例示を書いたら、囲みの記法でパースし直す（コメントならファイルを実行・import する、lint なら検出関数を直接走らせる）。目視では終端記号は読み飛ばされる。結合の受け入れ基準には各 unit の成功試験だけでなく、操作列の順序 assertion と接合部ごとの failure injection を置く。生成・削除・hash・publish・rollback をまたぐ契約は、最後に書くものと strict に失敗させるものを明示して検査する。
