...
- **名指しされた対処法が狙ったeffectを生まない trap**: 拒否messageや規約が示す手段を正常に起動できても、戻り値の成功と狙った状態変化を同一視すると、効かなかった対処を完了として次へ進む。
  ...
  後日の観測（#1201、2026-09-18）: 同じ guard の同じ deny について、名指しされた対処法（対象 worktree を root とする session を開き直す）は harness 側に実現手段が無いままだった。この回の処置は「確認 command を併記する」ではなく、決定そのものを deny から ask へ落とすことだった — 確認手段を書けない対処法しか無いなら、その分岐は人間へ渡す（上の対策の後半と同じ結論に、決定の格を下げる形で到達している）。この観測は当時の記録であり現行の指示ではない。
