...
閾値（20KB または 40項目、#797）を超えても exit code には出ない — 分割の判断は人間が行う通知であり CI を赤くする違反ではない。
機械化したのはこの規模判定だけで、残り2条件は未機械化のまま運用判断に委ねている:
直近5コミットの `git log --numstat` で純追記が3件以上を占めるか（改訂でなく追記のみが続いているか）、
main thread がカタログを直接読む経路が主になり全読が context を圧迫していると実測できたか、の2つ。
...
- **tagged** — 渡したタグのいずれかを持つもの。**一致タグ数の降順**に並び、各件の `matched N/M:` 行がそのサイクルのどのタグに当たって出てきたかを名指しする（同着はカタログ順のまま）。
  見出しは `tagged (N of M)` の形で、タグが選ぶ母数（always を除く全パターン）を常に併記する。
- **word-only** — 語だけが当てたもの。**タグが取りこぼした分がここに出る**。`--word` を渡さない回はその旨が出る。
  ...
  その「すでに tagged が持っていた分」がどのパターンかは reach 行からは分からない — `near --word <語>` が tagged 側も含めて名指しする。
