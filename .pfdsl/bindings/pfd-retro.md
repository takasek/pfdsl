...
- **解空間の継承 trap**: 作業項目が対応案を列挙していると、その集合をそのまま設計空間として受け取り、集合の外にある「問題自体が発生しない形」を誰も生成しない。
  ...
  **信号は3回出ていて3回とも検出されていた**（既存慣行への例外が要る・曖昧ケースの新設が要る・対称であるべき2方向の片方だけ挙動が違う）が、いずれも受容すべきコストとして分類された。
  ...
  対策: 「立ち止まれ」型の規則は効かない（信号は既に見えている）。規則は「比較対象を1個生成させる」形にする。
  却下理由の語彙制限（「手間」「大がかり」「今回のスコープ外」は却下理由として無効）が単体で最も効く。
  プロトコル本体への反映は #552。
...
- **不在判定の探索範囲 trap**: 「この識別子は実在しない（捏造だ）」と判断するとき、探索範囲が**その識別子が本来ありそうな場所**に限定され、**実際の出所**を含んでいないことがある。
  不在の証明は探索範囲がすべてであり、範囲を誤ると偽陽性が出る。
  存在の確認（1件見つければ足りる）と違って、不在の確認は範囲の網羅性そのものが結論を決める。
  問いの形: 「この識別子が『無い』と判断した検索範囲は、それが由来しうる場所を含んでいるか。同じファイル・同じテストの既存フィクスチャは見たか」。
  具体例: テストが `artifactKey: "ops_checkers"` を使っているのを見て、実在しない artifact キーの捏造と判断しかけた。
  ...
  対策: 不在判定の前に、対象ファイル自身と同種のテスト・フィクスチャを探索範囲に含める。「あるべき場所に無い」と「どこにも無い」を区別する。
