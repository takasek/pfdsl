...
- **フラグの意味範囲 trap**: フラグ名が示唆する狭い意味範囲（「今触っている検査の厳格版」）を信じて未検証のまま既存ゲートに組み込むと、そのフラグが実際に束ねる無関係な検査群が一斉に error 化し、意図しない大量破壊を CI で初めて知ることになる。
  ...
  **分割しただけでは半分しか直らない。分割後の名前が動作の主語を取り違えていると、同じ誤読がそのまま生き残る。** 上の例を最初 `--force-overwrite` / `--force-remove-orphans` に割ったが、後者は「orphan を削除するフラグ」と読める一方、実装が制御していたのは「ローカル編集を抱えた orphan も削除するか」だけで、編集の無い orphan はフラグ無しで既に削除されていた。
  つまり掃除が目的の人には**そもそも不要なフラグ**であり、#603 の当事者が渡した動機（旧パスを掃除したい）は名前によって作られていた。
