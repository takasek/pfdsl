...
## 削除を含むコミットの前に base 側の直近を読む

死んだコードを消す判断の根拠は「呼び出し元ゼロ」だが、base がサイクル途中で進むと、上流がその同じコードへ直前に投資していることがある。
投資は呼び出し元を増やさないので、根拠の再確認では現れない。
削除を含むコミットを作る前に、削除対象のパスへ `git log --oneline -5 origin/<base> -- <path>` を実行し、直近の変更が削除対象に触れていないか読む。
触れていれば、その変更が何を意図したかを読んでから削除の可否を決める。
rebase が競合を返さなかったことを判断の入力にしない — 競合するかは変更行が重なったかだけで決まり、上流が兄弟関数を直していれば無競合でマージされる。
