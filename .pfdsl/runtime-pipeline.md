...
- **c（PFD読み書き分析skill）**: `pfdsl_skill`。AIエージェントに`npx @pfdsl/cli <command>`を指示するスキル。自身のidentityは`tags: [c]`。呼び出し元b processには`tags: [invoked_by_c]`を付与して表現する（`>>`入力にはしない。理由は次節）
- **d（VSCode拡張）**: `vscode_extension`。CLIを介さずbを直接呼ぶ。自身のidentityは`tags: [d]`。呼び出し元b processには`tags: [invoked_by_d]`を付与して表現する（`export_metadata`は現状dのみが呼ぶ経路のため`tags: [b, invoked_by_d]`で`invoked_by_c`なし）。`pfdsl.preview`/hover/codelens/jump/document-link等のUI専用機能は検証可能な成果物を持たないためこのグラフの対象外（設計判断: 出力がエディタ内の一時表示のみで保存・再利用されない機能はartifact化しない）
...
`@pfdsl/cli`（cが指示する実行主体）と`vscode-extension`（d）は互いに依存しない。両者とも`@pfdsl/core`・`graphviz-exporter`・`preview-engine`を個別にimportし、`metadata-exporter`はvscode-extensionのみがimportする（`packages/cli/src/index.ts`と`packages/vscode-extension/src/*.ts`のimport文で確認済み）。

**呼び出し元は`>>`入力でなくtagで表現する。** `>>`は「processが依存する入力データ」を意味する。呼び出し元ソフトウェア（`pfdsl_skill`/`vscode_extension`）をb processへの`>>`入力にすると、「bがc/dに依存する」向きに逆転してしまう — 実際はc/dがbを呼ぶ側であり、bはc/dの存在を知らない。pfdslスキルの品質ガイド「横断的な性質はtagで束ねる」原則に従い、呼び出し元属性はb processの`tags`で表現し、`>>`エッジからは外す。この結果`pfdsl_skill`は`bundle_skills`（実ファイルコピー、真のデータ依存）のみに`>>`で繋がり、`vscode_extension`は本ファイル上孤立artifact（edgeなし）になる — spec.md上、孤立artifactは許容される宣言（孤立が禁止されるのはprocessのみ）。

**「呼び出し元」タグは`invoked_by_c`/`invoked_by_d`とし、a-gのレイヤーidentityタグ（`c`/`d`）とは別名前空間に分離する。** 当初`c`/`d`タグをそのままb processに流用した（`tags: [b, c, d]`）が、これはopusサブエージェントレビューで指摘を受けた — a-gタグは「そのノードがどのレイヤーに属するか（is-a）」を表す同一種類の分類軸であり、b processに`c`/`d`を付けると「processはbだがc/dレイヤーの一部でもある」という誤読を招く（実際に表したいのは「is-called-by」という別種の関係）。artifact側のidentityタグ（`pfdsl_skill: tags: [c]`）と紛れるため、呼び出し元専用タグ`invoked_by_c`/`invoked_by_d`を新設し分離した。

旧版（2つ前の修正）では`export_image`（cli固有groupと誤認）の入力に`vscode_extension`を混在させ、あたかもcliがextensionに依存するかのような構造になっていた。その修正として一旦「呼び出し元を全b processの`>>`入力に一律追加」する案を採ったが、これは同じ依存逆転を1箇所から全箇所へ拡大しただけの誤りだった。次にtag方式へ置き換えたが、レイヤーidentityタグをそのまま流用したため名前空間が混在した。最終的に`invoked_by_c`/`invoked_by_d`を分離して是正した。
