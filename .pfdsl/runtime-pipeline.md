...
- **f（PFD運用フレームワーク）**: このファイルの対象外。内容（L1+L2=一般層・L3=GitHub Issuesバックエンド層。L4はリポ固有で配布対象外・pd-ops自体には含まれない・retroフィードバック）の一次情報はworkflow.pfdslの`ops_skill_general`/`ops_skill_l3`。ここでは`bundle_skills`の入力（配布対象コンポーネント）としてのみ扱う
