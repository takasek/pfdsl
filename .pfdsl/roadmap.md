...
- どちらも `packages/cli/dist/cli.js` の存在を前提にする箇所がある（ビルドの前提は下の「worktree 前提」節）。`gate-check.mjs` はビルド未完了でも最後まで走り、項目名・SKIP 条件・canonical checklist への固定案内は通常どおり印字される（CLI に依存する `pfdsl check` と gen-plugin identity の2項目が FAIL になるだけ。実測 #560）
