...
**コード変更のあるサイクルはレビューを省略しない**: `packages/` または `scripts/` に変更がある回は、差分と関係する消費者を読み、品質と correctness を確認する。
通常は単独で1パス実施し、実装中に読んだだけでレビュー済みとは扱わない。
#561・#836 の実測は別の観点で再検査する価値を示したが、すべての変更で同じ agent 構成や本数を使う根拠にはしない。

1. **品質（簡素化・保守性）**: 不要な複雑さ、重複、既存の責務との整合を確認する。
2. **correctness**: 変更した挙動と事実主張に対し、偽になる入力・状態を試す。必要なら変更行の外の消費者も読む。
3. **設計妥当性**: 複数案から選んだ回は、採用理由が実装で成立するか確認する。案が複数あるだけで独立 agent を要求しない。
4. **体験（シナリオ実行）**: ユーザー可視の挙動・同梱内容を変える回は、現実的な利用例と変更が壊しうる既存動線を確認する。実行結果と原因の推測を分ける。

観点ごとに別パス・別 agent を立てる必要はない。
差分の不確実性、独立して調査できる範囲、読み手の文脈を分ける必要性から追加レビューの利点が説明できる場合に、範囲を絞って委譲する。
`/simplify` や `/code-review` は選択肢であり、固定の角度数や多段 fan-out は要求しない。
独立性そのものを検証する場合は解答を含まない資料・ツリーを渡し、自己レビューを独立レビューと呼ばない。
ユーザーが独立検証を完了条件にした回は、その条件を満たすまで未了として扱う。
公開時の配布読者検証は `distribution-review`、spec の write-probe が必要な場合は `spec-stress-test` の対象であり、通常レビューへ一律追加しない。

コード変更のレビュー記録はコミットの trailer に置く。
品質と correctness を単独で確認した回は次の1行で足りる。

```
Review: tool=self
```

`self` は検証を省略する印ではなく、差分の再読と反証確認を自分で実施した記録である。
追加のレビューを実施した場合だけ、その手段に対応する `Review:` 行も書く。
許容値とゲート充足条件の一次情報は `scripts/lib/review-record.mjs` の `REVIEW_TOOLS` / `GATE_TOOLS` / `CORRECTNESS_TOOLS`。
`self`、`correctness`、`design` はそれぞれ1行でコード変更の記録要件を満たす。
`simplify` だけでは correctness の確認を表さない。
散文・PFD のみの回は trailer を要求せず、内容に合った読戻し・構文検証を行う。
判定は従来どおり path 単位なので、`scripts/` 内の散文変更もコード変更の記録対象に含む。
レビューは commit 前に実施し、記録漏れだけを理由に履歴を書き換えない。
終端ゲートと CI は記録の形式を検査するが、実際のレビュー品質や独立性は保証しない。
...
- [ ] コード変更のあるサイクルでは、品質と correctness を確認し、実施方法に合ったレビュー記録を commit message に含めた。単独で確認した場合は `Review: tool=self` とする
