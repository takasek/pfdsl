...
**`/code-review` の実測期間中（#561 が open の間）**: 終端ゲートの `/simplify` または `/code-review` 項目を省略しない。
自己レビュー（差分の読み直し）は実施済みとみなし、それに**加えて**軽い設定の `/code-review`（角度を絞る。8角度 × 検証 agent の高効度設定は使わない）を実施する。
PR 本文のゲート節に3点を書く: 名指しした検査観点・自己レビューで気付いていなかった指摘が出たか（**出なかった場合も「なし」と明記する**）・出た指摘を採用したか。
記録先はファイルでなく PR 本文と #561 のコメントにする（並列 worktree での追記コンフリクトを避ける。ADR-0026 が同型の記録機構を廃止した理由と同じ）。
