...
- **選択フェーズ（pfd-ops 手順1）**: `GH_HOST=github.com node scripts/cycle-status.mjs` — fetch 実行・base への遅れコミット数・flow-sync PR / その他 open PR の一覧・`status ready --best` の結果を1回の JSON 出力に集約する。`--base <branch>` で対象ブランチを変更可能（デフォルト `main`）。加えて次の3点を出力する（#461）:
  ...
  - 対象 issue の本文・コメントを fetch し、設計確定状態を `designUnsettledFor` で出力する（#669）。実行前に知る必要があるのは対象 issue の決まり方だけで、`--issue <n>` が最優先、無ければ best プロセスの `location:` から解決し、どちらも得られない場合は判定を出さず `designUnsettledError` を返す。フィールドの構造と各値の意味はここに列挙しない（#913）— `scripts/lib/cycle-status.mjs` の `classifyDesignSettlement` の JSDoc が一次情報で、判定順・`reason` の4値・`unsettled` と `recordRequired` を別軸に分けた理由（#809・#833・#868）をそれ自身が述べる
  ...
  - このサイクルが着手する issue のうち、`flow:managed` なのに roadmap に process を持たないものを `unregisteredManagedIssues` で出力する（#963）。`audit-issues-flow.mjs` は同じ欠落を全 open issue について報告するが advisory 止まりで、監査を落とさない — 登録は実装ブランチに乗るため、そのブランチが main へマージされるまで他セッションからは常に欠落して見える。あるサイクルの差分が消せるのは自分が着手する issue の欠落だけで、他の issue の分は消せない。行動できるのがその1件だけなので、ここが唯一の検査点になる。非空で返ってきたら、着手前に roadmap へ依存チェーンを1本足すか、`flow:exempt` へ再分類するかのどちらかを済ませる — どちらでもないまま進めた回は、その issue の登録が誰の担当でもないまま残る
...
**実装を委譲する回はこの順序が構造的に反転する** — 委譲先はコミットまで済ませて戻るため、呼び出し元のレビューは必ずその後になる。
Review record ゲートはブランチ内の trailer を数えるので反転していても PASS し、緑からは見えない。
委譲するなら、レビューをどの時点で回すかをブリーフを書く時点で決める（委譲先にレビューまで含めるか、委譲先のコミットをレビュー前提の暫定と扱い呼び出し元が回してから改めてコミットするか）。
