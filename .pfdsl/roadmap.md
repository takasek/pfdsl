...
- **選択フェーズ（pfd-ops 手順1）**: `GH_HOST=github.com node scripts/cycle-status.mjs` — fetch 実行・base への遅れコミット数・flow-sync PR / その他 open PR の一覧・`ready --best` の結果を1回の JSON 出力に集約する。`--base <branch>` で対象ブランチを変更可能（デフォルト `main`）。加えて次の3点を出力する（#461）:
  - `openFlowSyncPRs` の各要素に `ci`（`PASS`/`FAIL`/`PENDING`/`NONE`/`UNKNOWN`）を含む。`gh pr checks` の別往復は不要
  - best 候補プロセスの issue 本文を fetch し、「design TBD」「設計未確定」等の設計未合意フレーズを検出した結果を `designUnsettled`（bool）・`designUnsettledLines`（ヒット行）で出力する。対称な複数案か事実上1案かの最終判断は引き続き AI が行う
  - best 候補プロセスの出力 artifact キーを `ready --json` の `outputs` フィールドから引き、実行すべき `gate-check.mjs --artifact <key>` の完成形コマンド行を `gateCheckCommand` で出力する（転記ミス・フォールバック判定への意図しない低下を防ぐ。roadmap.pfdsl 自前 regex パースは二重パースで構文変更に弱いため CLI 側の `outputs` フィールドを正とする）
- **終端ゲート機械9項目 + 報告材料2種（pfd-ops 手順3・#462）**: `GH_HOST=github.com node scripts/gate-check.mjs [--base main] [--artifact <key>]` — 内部で `git fetch origin` を試みたうえで `origin/<base>...HEAD` を基準に差分を取る（fetch 失敗時も既存 remote-tracking ref で続行し、ref 自体が無ければ明示エラーで終了する）。PASS/FAIL/SKIP 表で返す9項目: 変更 `.pfdsl` の `check` 通過・`audit-issues-flow` 差分なし・変更 `.md` の `check-md-linebreaks`・gen-plugin identity（該当変更時のみ）・snapshot 鮮度（`.pfdsl` 変更時のみ、`vitest` 自体の失敗も FAIL 扱い）・`roadmap.pfdsl` の `status:` 更新有無・vscode-extension typecheck（`packages/vscode-extension/` 変更時のみ）・commit subject の Conventional Commits lint・`roadmap.pfdsl` の wip 経由検証（todo→wip を実際に踏んだか。`--artifact` 省略時は粗いフォールバック）。`--artifact <key>` を渡すと status 更新・wip 経由の両方をその artifact に厳密スコープする（省略時はどちらも粗いフォールバック判定になる旨を detail に明示）。加えて2種の報告材料（PASS/FAIL でなく判断材料として印字）: 変更 `.pfdsl` ごとの新規終端 artifact リスト（`check --audit` 差分。手段か納品物かの分類のみ MANUAL に残る）・`roadmap.pfdsl` 変更時の ready-set 差分（newly ready / no longer ready、手順4の報告に使う）。判定不能な残り項目は `MANUAL:` prefix で列挙される — SKILL.md の終端ゲートチェックリストから実行時に抽出するため手打ちコピーは持たない。その項目のみ個別に確認する
...
- `gh` CLI が存在しない環境（Claude Code Remote / web セッション。GitHub 連携は `mcp__github__*` ツール経由のみ）では `audit-issues-flow` 項目が ENOENT で FAIL になる（#492）。この FAIL は同期ドリフトの検出ではなくツール不在によるものなので、`roadmap.pfdsl` の変更内容が status 更新のみ等ズレを生まない性質であることを目視確認したうえで N/A と判断してよい。
