...
記録を書く前に、L3 reference「設計確定の記法（`決定:` 行）」と `gate-check.mjs` の `DESIGN_RECORD_REQUIRED_PREFIXES` が課す書式を確認する。
終端ゲートはこの記録を機械的に読むため、実質を満たしていても行頭の形が合わなければ FAIL する。
書式を後から満たすには投稿済みコメントの編集になり、それは記録が着手前に存在したかどうかを外形から判定できなくする — ゲートが見ているのは投稿時刻であって編集時刻ではない。
