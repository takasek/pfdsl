...
- `gh` CLI が存在しない環境（Claude Code Remote / web セッション。GitHub 連携は `mcp__github__*` ツール経由のみ）での `audit-issues-flow` は経路が2つに分かれる（#489 で `gh-exec.mjs` に REST fallback を追加済み）。GH_TOKEN / GITHUB_TOKEN が無ければ ENOENT で FAIL する（#492）— この FAIL はツール不在によるもので同期ドリフトの検出ではないため、`roadmap.pfdsl` の変更が status 更新のみ等ズレを生まない性質であることを目視確認して N/A と判断してよい。token があれば REST fallback で実行されるが、この fallback は古い issue 番号を取りこぼし実在する open issue に対して false な `unknown_issue` finding を出すことがある（#543）。finding が実在 open issue（`mcp__github__issue_read` で確認）に対するもので、かつ今サイクルの `roadmap.pfdsl` 変更がその issue に触れていなければ、fallback の取りこぼしとして N/A と判断してよい。
