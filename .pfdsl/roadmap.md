...
diff の規模に合わせて委譲せず自分で読んだ回は、その手段の観点を借りていても `tool=` を書かない — 集計は tool を持たない記録を `unspecified` として別枠に置く。
手段名を書くと、委譲したパスが見つけた `new` の内訳に、自己レビューの直後にもう一度自分で読んだ回の件数が混ざる。
後者は構造的に `new` が小さいため、その手段の出現率を押し下げる。
