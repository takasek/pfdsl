...
- **同種対称性**: `published_cli` が持つ全消費エッジを `published_libraries` も持つか。`quality_guide` 新設時に `findings.criteria` の反映先列挙への追随が漏れた（読み直しで検出、2026-07-08）。`grill_skill`（#481 新設）が `retro_skill`・`ecosystem_skill` と同じ producer/consumer edge（`distill_ops` 出力・`publish_cli` 入力・`runtime-pipeline.pfdsl` の `assemble_plugin` 入力）を持つかの追随が初回セッションで漏れ、pfd-retro での本観点の適用で検出・修正した（2026-07-19）。配布スキルは同じ列挙が複数箇所に分散するため、新規追加時は既存の兄弟スキル名を grep して全参照箇所を機械列挙する運用に改めた（`.pfdsl/workflow.md`「配布スキルの新規追加時の横断照合」）
