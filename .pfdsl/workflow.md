...
レビューで「リテラルの分類を既存の宣言から導く」と提案するときは、その分類の出力がどの検査の入力になるかを調べる。
...
文書・設定の変更も、変更パス・キー・ラベルの参照から読み込み処理と値の利用先を追って判定する。新規・移動・削除では変更前後を調べ、確認した読取経路、または対象外と判断した探索範囲と根拠を、配布 work-cycle の記録要求と `.pfdsl/bindings/pfd-ops.md` の終端ゲートに従って PR 本文へ含める。既知パスとの不一致だけで対象外にしない。
...
3. **観点3 — 設計妥当性。** 条件付き発火 — このサイクルで実装・運用方式を競合案から実際に選んだ場合に実施する。設計記録に前提を外した案を義務として書いたことだけでは発火を決めない。ブリーフ要件は (a) 結論・採用理由を伏せて同じ設計問題を独立に解かせること（採用案を見せて攻めさせると提示解にアンカーされるため、検出機構の本体は敵対的姿勢でなく独立性に置く）、(b) 採用案の adoption rationale を名指しし、実装がそれを満たさない箇所を敵対的に探させること。観点2 の要件を含むため、発火した回は観点2 の別実行を要さない。
方式が既決でこのサイクルに新しい選択が無い場合は観点3を非発火とし、その理由を PR 本文に記す。発火した回は検討対象を絞って軽量化できるが、独立した別主体による検証は省かない。起動不能で実施できなかった回は非発火と扱わず、未実施としてユーザーへ返す。
...
**発火した観点は軽くしてよいが、消してはならない**: 発火条件と重さが別軸であること、軽量実施・非発火・未実施を分けて PR 本文に記すことは、同じ binding の終端ゲートが一次情報。
