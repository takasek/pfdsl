...
共通の観点1・2は最終差分の別 agent 1本で覆ってよい。観点3が発火する場合は、結論を知らない別 agent に解答を含まないツリーを渡して独立に設計させ、その後に最終差分の reviewer が採用理由と実装を確認する。観点4が発火する場合は変更意図を知らない別 agent にシナリオを渡す。観点を agent 数へ機械的に一対一対応させず、各観点の独立性を保つ。
自己レビューは準備として行い、別文脈のレビューの代替にしない。
指摘は根拠となる箇所と failure scenario を添えて返し、実装側が一次資料で確認して対応する。
リテラルや特殊ケースの分類を既存の宣言・データから導く指摘を採用する前には、この節の「両ホストでの確認」へ戻る。
各観点の状態、確認範囲・根拠・未確認、未解決の指摘を PR 本文に記録する。
...
Claude Code では共通の観点と発火条件を、利用可能な skill・Agent tool に割り当てる。Codex の通常の調査・実装の単独実行方針は、Claude Code の委譲手段には適用しない。
観点1 は `/simplify` の固定4角度で確認する。ただし `/simplify` は correctness を明示的に探さないため、観点2を兼ねたとは扱わない。
観点2 は反証と消費者探索をブリーフした subagent で確認する。`/code-review` も観点2の要件を実際に満たせる範囲で使い、不足する要件は別のレビューで補う。観点3 の独立解では解答を含まないツリーを Agent tool で別主体に渡し、最終差分の reviewer と分ける。観点4 は成果物とシナリオだけを渡した subagent、または領域に対応する専用 skill で確認する。
#561 の48サイクルでは自己レビューで気付かなかった指摘が85%のサイクルで出た。#836 では `/simplify` に findings が無い回にも別観点が採用理由の不成立と JSDoc の誤りを検出した。これらは共通の観点と別主体の必要性を支える実測であり、skill ごとの起動方法を他ホストへ要求する根拠ではない。
...
起動可否が harness と plugin の版に依存し、記録された「起動できない」がその時点の観測でしかないことは同じ binding の終端ゲートが一次情報。このリポで確認する実体フィールドは `disable-model-invocation` で、2026-07-28 時点の `/code-review` は `disable-model-invocation: false`。
