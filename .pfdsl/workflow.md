...
次セッションへ渡す手順には、**壊れたことの確認を手順そのものに含める**。
`git stash push -- <file>` のように作業ツリーの差分を前提にする壊し方は、その差分をコミットした後は「No local changes to save」で空振りし、以降の検査は健全なツリーに対して走って緑を返す（#754 の検証で実測。手順を書いた時点では未コミットだった操作を、コミット後に実行する形でそのまま渡していた）。
空振りは失敗でなく緑として出るため、確認を挟まない限り検証者からは成功と区別できない。
壊した直後に「期待した検査が実際に赤いこと」を1度確認させてから本題に入る。
