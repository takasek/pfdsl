...
文書・設定の変更も、変更パス・キー・ラベルの参照から読み込み処理と値の利用先を追って判定する。新規・移動・削除では変更前後を調べ、確認した読取経路、または対象外と判断した探索範囲と根拠を、`.pfdsl/bindings/pfd-ops.md` の終端ゲートに従って PR 本文へ含める。配布 work-cycle は記録先の印を扱う一般規則を持ち、レビュー固有の項目は持たない。既知パスとの不一致だけで対象外にしない。
...
2. **観点2 — correctness。** コード変更と、機械が読む文書・設定の抽出対象を変えるサイクルで担保する。ブリーフ要件は (a) diff が導入・変更した事実主張（コメント・JSDoc・doc 散文・criteria 文言）を列挙し、各主張の**反証を試みる**こと（真偽判定でなく偽になる入力・状態を構成させる — 追認バイアスを falsification に固定するため）、(b) 変更行の外の消費者（散文を含む）を、`.pfdsl/bindings/pfd-ops.md` の終端ゲートが求める影響先確認と終了判断に従って調べること。条件撤去では `.pfdsl/bindings/` のカタログ・運用文書も探索に含める。軽量 subagent 1本を想定する。
3. **観点3 — 設計妥当性。** 条件付き発火 — このサイクルで実装・運用方式を新たに選ぶか、着手前に確定していた方式を変更した場合に実施する。着手前の issue 記録・ユーザー指示・既存規約で方式が確定し、その実装だけを行う回は非発火である。適用点1が義務として求める「前提を外した案」や、そこから「何もしない」に至ったため追加した競合実装案を記録・比較・却下したことだけでは、新しい方式選択と数えない。ブリーフ要件は (a) 結論・採用理由を伏せて同じ設計問題を独立に解かせること（採用案を見せて攻めさせると提示解にアンカーされるため、検出機構の本体は敵対的姿勢でなく独立性に置く）、(b) 採用案の adoption rationale を名指しし、実装がそれを満たさない箇所を敵対的に探させること。観点2 の要件を含むため、発火した回は観点2 の別実行を要さない。
方式が既決でこのサイクルに新しい選択が無い場合は観点3を非発火とし、その根拠となる着手前の記録・指示・規約を PR 本文に記す。発火した回は検討対象を絞って軽量化できるが、独立した別主体による検証は省かない。起動不能で実施できなかった回は非発火と扱わず、未実施としてユーザーへ返す。
...
いずれのレビュー手段から受けた指摘でも、リテラルや特殊ケースの分類を既存の宣言・データから導く提案を採用する前には、この節の「両ホストでの確認」へ戻る。

**全観点共通**: finding に failure scenario を伴わせることは、`.pfdsl/bindings/pfd-ops.md`「終端ゲートの追加項目を検査して完了を確認する」のレビュー項目が一次情報。

**発火した観点は軽くしてよいが、消してはならない**: Claude Code の観点 menu について、発火条件と重さが別軸であること、軽量実施・非発火・未実施を分けて PR 本文に記すことは、同じ binding の終端ゲートが一次情報。
このリポでのインスタンス: 散文のみのサイクルでも条件付き観点は発火しうる — 配布同梱物の散文変更は観点4 の発火条件を満たす。
担保の中核が委譲でしか作れない観点を軽くできないこと・起動できない回はユーザーへ返すこと・起動可否の判断自体を指示の文面で確かめることも、同じ binding の終端ゲートが持つ。
